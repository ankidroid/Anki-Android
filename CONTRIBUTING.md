...
1. Reproduce the issue. Comment with the version number/debug info if you're questing whether it's 
  current. If you can't reproduce it:
...
1. Ensure you branched off `main` recently and are submitting from a branch on your fork (not `main`).
    * If you submit a PR from `main`, it becomes difficult to submit multiple  
2. Please run these checks before submitting a PR: [workflows/README.md](https://github.com/ankidroid/Anki-Android/tree/main/.github/workflows/README.md#quality-checks). Your PR will not be reviewed until automated checks 
   using GitHub Actions pass.
3. Check commits/diffs
...
4. Write a PR description
    * Our [PR template]((https://github.com/david-allison/Anki-Android/blob/contributing-md/.github/pull_request_template.md)) **must** be filled (unless trivial). Create PRs using GitHub so this template will be pre-populated for you to submit.

Writing automated tests helps us with confident that your change is correct. This is currently 
...
> [!IMPORTANT]
> Don't close a PR if something unexpected happened, use `git push --force` to fix it, or get in touch.
...
If a **suggestion** is provided, feel free to apply it in the GitHub UI. Ideally `rebase` & `squash` it 
 locally and `force push` the branch to keep the history clean

Sometimes a **patch** is provided. See [Wiki: Applying Patches](https://github.com/ankidroid/Anki-Android/wiki/Development-Guide/#applying-a-patch) 
...
If you feel your PR is no longer reviewable, [mark it as a draft](https://docs.github.com/en/pull-requests/collaborating-with-pull-requests/proposing-changes-to-your-work-with-pull-requests/changing-the-stage-of-a-pull-request#converting-a-pull-request-to-a-draft) until changes are made.
...
Feel free to select a new issue. Once you've done a handful of '**Good First Issues**', we hope you 
 feel that most of the issues in the repo are within your grasp. 
