...
class FixedPreferencesTitleLength : ResourceXmlDetector(), XmlScanner {
    ...
    override fun visitElement(context: XmlContext, element: Element) {
        if ("xml" == context.file.parentFile.name && element.hasAttribute(ATTR_TITLE)) {
            ...
            if (!titleAttribute.startsWith("@string/")) {
                // Ignore strings that are hard-coded. That's the case with dev options.
                return
            }
            val stringName = titleAttribute.substringAfter("@string/", "")
    ...
    override fun afterCheckEachProject(context: Context) {
        for (title in titlesOfPreferenceScreens) {
            val stringHandle = stringResources[title]!!
            ...
            } else if (stringElement.getAttribute(ATTR_MAX_LENGTH) > PREFERENCES_TITLE_MAX_LENGTH.toString()) {
