...
In order to fix existing images, there is a [python script](https://github.com/mrestko/image-occlusion-svg-fix) created by Matt Restko which you can run. Please ensure that you have python installed (either 2.7.x or 3.x should be fine), and follow the instructions on his github repository. Please kindly understand that we cannot directly provide support on this issue, as it's not an AnkiDroid issue.
