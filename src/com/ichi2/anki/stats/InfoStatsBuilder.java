...
public class InfoStatsBuilder {
    ...
    private void appendTodaysStats(StringBuilder stringBuilder){
        ...
        if(todayStats[CARDS_INDEX] > 0) {
            ...
            stringBuilder.append(mWebView.getResources().getString(R.string.stats_today_correct_count, (((1 - todayStats[FAILED_INDEX] / (float) (todayStats[CARDS_INDEX])) * 100.0))));
        }
        stringBuilder.append("<br>");
        stringBuilder.append(mWebView.getResources().getString(R.string.stats_today_type_breakdown, todayStats[LRN_INDEX], todayStats[REV_INDEX], todayStats[RELRN_INDEX], todayStats[FILT_INDEX]));
        stringBuilder.append("<br>");
        if(todayStats[MCNT_INDEX] != 0){
            stringBuilder.append(mWebView.getResources().getString(R.string.stats_today_mature_cards, todayStats[MSUM_INDEX], todayStats[MCNT_INDEX], (todayStats[MSUM_INDEX] / (float)(todayStats[MCNT_INDEX]) * 100.0)));
        } else
            stringBuilder.append(mWebView.getResources().getString(R.string.stats_today_no_mature_cards));
    }
