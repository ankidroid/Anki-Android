...
public class Reviewer extends Activity {
	...
	private static final Pattern sSpanPattern = Pattern
			.compile("</?span[^>]*>");
	...
	private boolean mUpdateNotifications; // TODO use Veecheck only if this is
											// true
	...
	private static final int DICTIONARY_LEO_WEB = 1; // German web dictionary
														// for English, French,
														// Spanish, Italian,
														// Chinese, Russian
	private static final int DICTIONARY_LEO_APP = 2; // German web dictionary
														// for English, French,
														// Spanish, Italian,
														// Chinese, Russian
	...
	private DeckTask.TaskListener mMarkCardHandler = new DeckTask.TaskListener() {
		...
		public void onPreExecute() {
			...
			mProgressDialog = ProgressDialog.show(Reviewer.this, "",
	...
	private DeckTask.TaskListener mAnswerCardHandler = new DeckTask.TaskListener() {
		...
		public void onProgressUpdate(DeckTask.TaskData... values) {
			...
			if ((sessionRepLimit > 0) && (mSessionCurrReps >= sessionRepLimit)) {
				...
				sessionMessage = Toast.makeText(Reviewer.this,
						res.getString(R.string.session_question_limit_reached),
						Toast.LENGTH_SHORT);
			} else if ((sessionTime > 0)
					&& (System.currentTimeMillis() >= mSessionTimeLimit)) {
				// session time limit reached, flag for halt once async task has
				// completed.
				mSessionComplete = true;
				sessionMessage = Toast.makeText(Reviewer.this,
						res.getString(R.string.session_time_limit_reached),
						Toast.LENGTH_SHORT);
			} else if (mIsLastCard) {
				mNoMoreCards = true;
				...
				// If the card is null means that there are no more cards
				// scheduled for review.
				if (newCard == null) {
					mNoMoreCards = true;
					mProgressDialog = ProgressDialog.show(Reviewer.this, "",
							getResources().getString(R.string.saving_changes),
							true);
					return;
				}
		...
		@Override
		public void onPostExecute(DeckTask.TaskData result) {
			// Check for no more cards before session complete. If they are both
			// true,
			// no more cards will take precedence when returning to study
			// options.
			if (mNoMoreCards) {
		...
		if (deck == null) {
			...
			if (mPrefFullscreenReview) {
				getWindow().setFlags(
						...
						WindowManager.LayoutParams.FLAG_FULLSCREEN);
				// Do not hide the title bar in Honeycomb, since it contains the
				// action bar.
			...
			case DICTIONARY_AEDICT:
				...
				mIsDictionaryAvailable = Utils.isIntentAvailable(this,
						mDictionaryAction);
				break;
			...
			case DICTIONARY_LEO_APP:
				...
				mIsDictionaryAvailable = Utils.isIntentAvailable(this,
						mDictionaryAction, new ComponentName(
			...
			Log.i(AnkiDroidApp.TAG, "Is intent available = "
					+ mIsDictionaryAvailable);

			// Load the template for the card and set on it the available width
			// for images
			try {
				mCardTemplate = Utils.convertStreamToString(getAssets().open(
						"card_template.html"));
				mCardTemplate = mCardTemplate.replaceFirst(
						"var availableWidth = \\d*;", "var availableWidth = "
			...
			// Log.i(AnkiDroidApp.TAG, "SessionTimeLimit: " + timelimit +
			// " ms.");
			...
			if (mPrefWhiteboard
					&& MetaDB.getWhiteboardState(this, mDeckFilename) == 1) {
			...
			DeckTask.launchDeckTask(DeckTask.TASK_TYPE_ANSWER_CARD,
					mAnswerCardHandler, new DeckTask.TaskData(0, deck, null));
		}
	}
	...
	public void onConfigurationChanged(Configuration newConfig) {
		...
		// Modify the card template to indicate the new available width and
		// refresh card
		mCardTemplate = mCardTemplate.replaceFirst(
				"var availableWidth = \\d*;", "var availableWidth = "
						+ getAvailableWidthInCard() + ";");
		...
		// Also skipping the counts (because we don't know which one to
		// underline)
	...
	public boolean onCreateOptionsMenu(Menu menu) {
		...
		if (mPrefWhiteboard) {
			if (mShowWhiteboard) {
				Utils.addMenuItemInActionBar(menu, Menu.NONE, MENU_WHITEBOARD,
						Menu.NONE, R.string.hide_whiteboard,
						R.drawable.ic_menu_compose);
			} else {
				Utils.addMenuItemInActionBar(menu, Menu.NONE, MENU_WHITEBOARD,
						Menu.NONE, R.string.show_whiteboard,
						R.drawable.ic_menu_compose);
			}
			Utils.addMenuItemInActionBar(menu, Menu.NONE,
					MENU_CLEAR_WHITEBOARD, Menu.NONE,
					R.string.clear_whiteboard,
					R.drawable.ic_menu_clear_playlist);
		}
		Utils.addMenuItem(menu, Menu.NONE, MENU_EDIT, Menu.NONE,
				R.string.menu_edit_card, R.drawable.ic_menu_edit);

		SubMenu removeDeckSubMenu = menu.addSubMenu(Menu.NONE, MENU_REMOVE,
				Menu.NONE, R.string.menu_remove_card);
		removeDeckSubMenu.setIcon(R.drawable.ic_menu_stop);
		removeDeckSubMenu.add(Menu.NONE, MENU_REMOVE_BURY, Menu.NONE,
				R.string.menu_bury_card);
		removeDeckSubMenu.add(Menu.NONE, MENU_REMOVE_SUSPEND, Menu.NONE,
				R.string.menu_suspend_card);
		removeDeckSubMenu.add(Menu.NONE, MENU_REMOVE_DELETE, Menu.NONE,
		...
		item = menu.add(Menu.NONE, MENU_MARK, Menu.NONE,
				R.string.menu_mark_card);
		Utils.addMenuItemInActionBar(menu, Menu.NONE, MENU_UNDO, Menu.NONE,
				R.string.undo, R.drawable.ic_menu_revert);
		Utils.addMenuItemInActionBar(menu, Menu.NONE, MENU_REDO, Menu.NONE,
				R.string.redo, R.drawable.ic_menu_redo);
		return true;
	}
	...
	public boolean onPrepareOptionsMenu(Menu menu) {
		...
		if (mPrefTextSelection) {
			...
			Log.i(AnkiDroidApp.TAG,
					"Clipboard has text = " + mClipboard.hasText());
		...
		menu.findItem(MENU_UNDO)
				.setEnabled(AnkiDroidApp.deck().undoAvailable());
		menu.findItem(MENU_REDO)
				.setEnabled(AnkiDroidApp.deck().redoAvailable());
		return true;
	}
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		case MENU_REMOVE_BURY:
			DeckTask.launchDeckTask(DeckTask.TASK_TYPE_BURY_CARD,
					mAnswerCardHandler,
...
//								AnkiDroidApp.deck(), mCurrentCard));
			...
			mUnmountReceiver = new BroadcastReceiver() {
				...
				public void onReceive(Context context, Intent intent) {
					...
					if (action.equals(Intent.ACTION_MEDIA_EJECT)) {
						Log.i(AnkiDroidApp.TAG,
								"mUnmountReceiver - Action = Media Eject");
						finishNoStorageAvailable();
					}
				}
			};
	...
	private String getLanguage(int questionAnswer) {
		String language = MetaDB.getLanguage(this, mDeckFilename, mCurrentCard
				.getModel().getId(), mCurrentCard.getOrd(), questionAnswer);
		return language;
	}

	private void storeLanguage(String language, int questionAnswer) {
		MetaDB.storeLanguage(this, mDeckFilename, mCurrentCard.getModel()
				.getId(), mCurrentCard.getOrd(), questionAnswer, language);
	}
	...
	private boolean editCard() {
		if (isCramming()) {
			Toast cramEditWarning = Toast.makeText(Reviewer.this,
					getResources().getString(R.string.cram_edit_warning),
					Toast.LENGTH_SHORT);
	...
	private boolean lookUp() {
		if (mPrefTextSelection && mClipboard.hasText()
			...
			case DICTIONARY_AEDICT:
				...
				return true;
			case DICTIONARY_LEO_WEB:
				// localisation is needless here since leo.org translates only
				// into or out of German
				final CharSequence[] itemValues = { "en", "fr", "es", "it",
						"ch", "ru" };
				...
				for (int i = 0; i < itemValues.length; i++) {
					if (language.equals(itemValues[i])) {
						Intent leoSearchIntent = new Intent(mDictionaryAction,
								Uri.parse("http://pda.leo.org/?lp=" + language
										+ "de&search=" + mClipboard.getText()));
						startActivity(leoSearchIntent);
						mClipboard.setText("");
						return true;
					}
				}
				final CharSequence[] items = { "Englisch", "Französisch",
						"Spanisch", "Italienisch", "Chinesisch", "Russisch" };
				AlertDialog.Builder builder = new AlertDialog.Builder(this);
				builder.setTitle("\"" + mClipboard.getText()
						+ "\" nachschlagen");
			...
			case DICTIONARY_LEO_APP:
				Intent leoSearchIntent = new Intent(mDictionaryAction);
				leoSearchIntent.putExtra(Intent.EXTRA_TEXT,
						mClipboard.getText());
				leoSearchIntent
						.setComponent(new ComponentName("org.leo.android.dict",
								"org.leo.android.dict.LeoDict"));
				startActivity(leoSearchIntent);
				mClipboard.setText("");
				return true;
			case DICTIONARY_COLORDICT:
				...
				colordictSearchIntent.putExtra("EXTRA_QUERY",
						mClipboard.getText());
	...
	private void showDeleteCardDialog() {
		...
		builder.setMessage(String.format(
				...
				Utils.stripHTML(mCurrentCard.getQuestion()),
				Utils.stripHTML(mCurrentCard.getAnswer())));
		builder.setPositiveButton(res.getString(R.string.yes),
				new DialogInterface.OnClickListener() {
					@Override
					public void onClick(DialogInterface dialog, int which) {
						DeckTask.launchDeckTask(DeckTask.TASK_TYPE_DELETE_CARD,
								mAnswerCardHandler, new DeckTask.TaskData(0,
										AnkiDroidApp.deck(), mCurrentCard));
					}
				});
	...
	private void answerCard(int ease) {
		...
		mTimerHandler.postDelayed(removeChosenAnswerText,
				mShowChosenAnswerLength);
		Sound.stopSounds();
		mCurrentEase = ease;
		// Increment number reps counter
		mSessionCurrReps++;
		DeckTask.launchDeckTask(DeckTask.TASK_TYPE_ANSWER_CARD,
				mAnswerCardHandler, new DeckTask.TaskData(mCurrentEase, deck,
						mCurrentCard));
	}
	...
	private void initLayout(Integer layout) {
		...
		mNextTimeTextColor = getResources().getColor(
				R.color.next_time_usual_color);
		mNextTimeTextRecomColor = getResources().getColor(
				R.color.next_time_recommended_color);
	...
	private void invertColors() {
		...
		mNextTimeTextRecomColor = res
				.getColor(R.color.next_time_recommended_color_inv);
		...
		mFlipCard.setBackgroundDrawable(res
				.getDrawable(R.drawable.btn_keyboard_key_fulltrans_normal));
		mEase1.setBackgroundDrawable(res
				.getDrawable(R.drawable.btn_keyboard_key_fulltrans_normal));
		mEase2.setBackgroundDrawable(res
				.getDrawable(R.drawable.btn_keyboard_key_fulltrans_normal));
		mEase3.setBackgroundDrawable(res
				.getDrawable(R.drawable.btn_keyboard_key_fulltrans_normal));
		mEase4.setBackgroundDrawable(res
				.getDrawable(R.drawable.btn_keyboard_key_fulltrans_normal));
	...
	private void hideEaseButtons() {
		...
		// INVISIBLE -> The transition between the question and the answer seems
		// more smooth
	...
	private void initControls() {
		...
		if (mPrefWhiteboard) {
			mWhiteboard.setVisibility(mShowWhiteboard ? View.VISIBLE
					: View.GONE);
		}
		mAnswerField.setVisibility((mPrefWriteAnswers) ? View.VISIBLE
				: View.GONE);
	}
	...
	private SharedPreferences restorePreferences() {
		...
		mPrefFullscreenReview = preferences
				.getBoolean("fullscreenReview", true);
		mshowNextReviewTime = preferences
				.getBoolean("showNextReviewTime", true);
		mZoomEnabled = preferences.getBoolean("zoom", false);
		mDisplayFontSize = preferences.getInt("relativeDisplayFontSize", 100);
		mRelativeButtonSize = preferences.getInt("answerButtonSize", 100);
		mPrefHideQuestionInAnswer = Integer.parseInt(preferences.getString(
				"hideQuestionInAnswer", Integer.toString(HQIA_DO_SHOW)));
		...
		if (mGesturesEnabled) {
			mGestureShake = Integer.parseInt(preferences.getString(
					"gestureShake", "0"));
			if (mGestureShake != 0) {
				mShakeEnabled = true;
			}
			...
			mGestureSwipeUp = Integer.parseInt(preferences.getString(
					"gestureSwipeUp", "0"));
			mGestureSwipeDown = Integer.parseInt(preferences.getString(
					"gestureSwipeDown", "0"));
			mGestureSwipeLeft = Integer.parseInt(preferences.getString(
					"gestureSwipeLeft", "13"));
			mGestureSwipeRight = Integer.parseInt(preferences.getString(
					"gestureSwipeRight", "0"));
			mGestureDoubleTap = Integer.parseInt(preferences.getString(
					"gestureDoubleTap", "0"));
			mGestureTapLeft = Integer.parseInt(preferences.getString(
					"gestureTapLeft", "0"));
			mGestureTapRight = Integer.parseInt(preferences.getString(
					"gestureTapRight", "0"));
			mGestureTapTop = Integer.parseInt(preferences.getString(
					"gestureTapTop", "0"));
			mGestureTapBottom = Integer.parseInt(preferences.getString(
					"gestureTapBottom", "0"));
		}
	...
	private void setDueMessage() {
		// if (mCurrentCard != null &&
		// AnkiDroidApp.deck().getScheduler().equals("reviewEarly")) {
		// double due = (mCurrentCard.getCombinedDue() - Utils.now()) / 86400.0;
		// if (due > 0.041) {
		// mChosenAnswer.setText(Utils.getReadableInterval(Reviewer.this, due,
		// true));
		// }
		// }
	}
	...
	private void displayCardQuestion() {
		...
		if (mPrefWriteAnswers) {
			...
			inputMethodManager.showSoftInput(mAnswerField,
		...
		if (mSpeakText && Integer.valueOf(android.os.Build.VERSION.SDK) > 3) {
			ReadText.setLanguageInformation(mCurrentCard.getOrd(),
					mCurrentCard.getOrd());
		}
	...
	private void displayCardAnswer() {
		...
		if (mPrefWriteAnswers) {
			...
			if (mCurrentCard != null) {
				...
				Matcher matcher = sSpanPattern.matcher(Utils
						.stripHTMLMedia(mCurrentCard.getAnswer()));
				...
				displayString = enrichWithQASpan(
						diff.diff_prettyHtml(diff.diff_main(userAnswer,
								correctAnswer))
								+ "<br/>"
								+ mCurrentCard.getAnswer(), true);
			}

			// Hide soft keyboard
			InputMethodManager inputMethodManager = (InputMethodManager) getSystemService(Context.INPUT_METHOD_SERVICE);
			inputMethodManager.hideSoftInputFromWindow(
					mAnswerField.getWindowToken(), 0);
		} else {
	...
	private void updateCard(String content) {
		...
		if (mCurrentCard != null) {
			...
			// Model myModel = Model.getModel(currentDeck,
			// mCurrentCard.getOrd(), false);
			// baseUrl = Utils.getBaseUrl(mMediaDir, myModel, currentDeck);
			content = mCurrentCard.getModel().getCSSForFontColorSize(
					mCurrentCard.getOrd(), mDisplayFontSize, mInvertedColors)
					+ content;
			// isJapaneseModel = myModel.hasTag(japaneseModelTag);
			// mMainLayout.setBackgroundColor(Color.parseColor(myModel.getBackgroundColor(mCurrentCard.getOrd(),
			// mInvertedColors)));
		} else {
			mCard.getSettings().setDefaultFontSize(
					calculateDynamicFontSize(content));
			baseUrl = Utils.urlEncodeMediaDir(mDeckFilename.replace(".anki",
					".media/"));
		}
		...
		int questionStartsAt = content
				.indexOf("<a name=\"question\"></a><hr/>");
		if (isQuestionDisplayed()) {
			if (sDisplayAnswer && (questionStartsAt != -1)) {
				content = Sound.parseSounds(baseUrl,
						content.substring(0, questionStartsAt), mSpeakText,
						MetaDB.LANGUAGE_QUESTION)
						+ Sound.parseSounds(
								baseUrl,
								content.substring(questionStartsAt,
										content.length()), mSpeakText,
								MetaDB.LANGUAGE_ANSWER);
			} else {
				content = Sound.parseSounds(baseUrl,
						content.substring(0, content.length() - 5), mSpeakText,
						MetaDB.LANGUAGE_QUESTION) + "<hr/>";
			}
		} else {
		...
		String card = mCardTemplate.replace("::content::", content).replace(
		...
		if (!mConfigurationChanged && mPlaySoundsAtStart) {
			if (!mSpeakText) {
				...
				Sound.playSounds(Utils.stripHTML(mCurrentCard.getQuestion()),
						MetaDB.LANGUAGE_QUESTION);
			} else {
				Sound.playSounds(Utils.stripHTML(mCurrentCard.getAnswer()),
						MetaDB.LANGUAGE_ANSWER);
			}
		}
	}
	...
	private String getDeckStyle(String deckPath) {
		...
		StringBuilder style = new StringBuilder();
		try {
			BufferedReader styleReader = new BufferedReader(
					new InputStreamReader(new FileInputStream(styleFile)));
			while (true) {
			...
			Log.e(AnkiDroidApp.TAG,
					"Error reading style file: " + styleFile.getAbsolutePath(),
					e);
			return "";
		}
	 ...
	 * <p>
	 * Custom fonts live in fonts directory in the directory used to store
	 * decks.
	 * <p>
	 * Each font is mapped to the font family by the same name as the name of
	 * the font fint without the extension.
	 */
	private String getCustomFontsStyle() {
		StringBuilder builder = new StringBuilder();
		for (File fontFile : Utils.getCustomFonts(getBaseContext())) {
			String fontFace = String
					.format("@font-face {font-family: \"%s\"; src: url(\"file://%s\");}",
							Utils.removeExtension(fontFile.getName()),
	...
	private int getAvailableWidthInCard() {
		...
		int availableWidth = (int) (AnkiDroidApp.getDisplayWidth() / mScaleInPercent)
				- TOTAL_WIDTH_PADDING;
	 ...
	 * the clipboard. From
	 * http://cosmez.blogspot.com/2010/04/webview-emulateshiftheld
	 * -on-android.html
	 */
	...
	/**
	 * Setup media. Try to detect if we're using dropbox and set the mediaPrefix
	 * accordingly. Then set the media directory.
	 * 
	 * @param deck
	 *            The deck that we've just opened
	 */
	...
	private void executeCommand(int which) {
		...
		case GESTURE_REDO:
			if (AnkiDroidApp.deck().redoAvailable()) {
				DeckTask.launchDeckTask(DeckTask.TASK_TYPE_REDO,
						mUpdateCardHandler, new DeckTask.TaskData(0,
								AnkiDroidApp.deck(), mCurrentCard.getId(),
								false));
			}
			break;
		case GESTURE_EDIT:
	...
	private void closeReviewer() {
		mClosing = true;
		DeckTask.launchDeckTask(DeckTask.TASK_TYPE_SAVE_DECK,
				mSaveAndResetDeckHandler,
				new DeckTask.TaskData(AnkiDroidApp.deck(), ""));
	}
				...
				try {
					if (e2.getY() - e1.getY() > StudyOptions.sSwipeMinDistance
						...
						executeCommand(mGestureSwipeUp);
							...
							&& Math.abs(e1.getY() - e2.getY()) < StudyOptions.sSwipeMaxOffPath
