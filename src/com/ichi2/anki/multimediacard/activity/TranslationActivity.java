 ...
 */
public class TranslationActivity extends FragmentActivity implements
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (savedInstanceState != null) {
			boolean b = savedInstanceState.getBoolean(BUNDLE_KEY_SHUT_OFF,
					false);
			if (b) {
				finishCancel();
				return;
	...
	private String computeAddress() {
		...
		address = address.replaceAll("FROMLANG", langCodeFrom)
				.replaceAll("TOLANG", mLangCodeTo).replaceAll("SOURCE", query);
	...
	private void showPickTranslationDialog() {
		if (mTranslation.startsWith("FAILED")) {
			returnFailure(getText(
					R.string.multimedia_editor_trans_getting_failure)
					.toString());
			return;
		}
		...
		if (!resp.getResult().contentEquals("ok")) {
			if (!mSource.toLowerCase(Locale.getDefault())
					.contentEquals(mSource)) {
		...
		if (mPossibleTranslations.size() == 0) {
			...
			returnFailure(getText(
					R.string.multimedia_editor_error_word_not_found).toString());
			return;
		}
		...
		fragment.setTitle(getText(
				R.string.multimedia_editor_trans_pick_translation).toString());

		fragment.show(this.getSupportFragmentManager(), "pick.translation");

	}

	private static ArrayList<String> parseJson(Response resp,
		 ...
		 * reader's understanding. We leave here only the translations to the
		...
		String desiredLang = LanguagesListerGlosbe
				.requestToResponseLangCode(languageCodeTo);
		...
		for (Tuc tuc : tucs) {
			...
			if (meanings != null) {
				for (Meaning meaning : meanings) {
					...
					if (meaning.getLanguage().contentEquals(desiredLang)) {
						String unescappedString = HtmlUtil.unescape(meaning
								.getText());
			...
			if (phrase != null) {
				...
				if (phrase.getLanguage().contentEquals(desiredLang)) {
					String unescappedString = HtmlUtil.unescape(phrase
							.getText());
					res.add(unescappedString);
				}
			}
