...
public class DeckPicker extends AnkiControllableFragmentActivity
{
    ...
    boolean mCompletionBarRestrictToActive = false; // set this to true in order
        ...
        public void onClick(DialogInterface dialog, int item)
        {
            ...
            switch (item)
            {
                ...
                case CONTEXT_MENU_CUSTOM_DICTIONARY:
                    // String[] dicts =
                    // res.getStringArray(R.array.dictionary_labels);
                    // String[] vals =
                    // res.getStringArray(R.array.dictionary_values);
                    // int currentSet =
                    // MetaDB.getLookupDictionary(DeckPicker.this,
                    // data.get("filepath"));
                    //
                    ...
                    // int currentChoice = 0;
                    // labels[0] =
                    // res.getString(R.string.deckpicker_select_dictionary_default);
                    // mDictValues[0] = -1;
                    ...
                    // StyledDialog.Builder(DeckPicker.this);
                    // builder.setTitle(res.getString(R.string.deckpicker_select_dictionary_title));
                    // builder.setSingleChoiceItems(labels, currentChoice, new
                    // DialogInterface.OnClickListener() {
                    // public void onClick(DialogInterface dialog, int item) {
                    // MetaDB.storeLookupDictionary(DeckPicker.this,
                    // mCurrentDeckPath, mDictValues[item]);
                    // }
                    // });
                ...
                case CONTEXT_MENU_DECK_SUMMARY:
                    ...
                    // mLoadStatisticsHandler, new
                    // DeckTask.TaskData(DeckPicker.this, new
                    // String[]{data.get("filepath")}, mStatisticType, 0));
                    return;
            }
        }
    };
            ...
            if (!data.success)
            {
                ...
                if (result[0] instanceof String)
                {
                    ...
                    else if (resultType.equals("clockOff"))
                    {
                        ...
                        {
                            // The difference would be within limit if we
                            // adjusted the time by few hours
                            // It doesn't work for all timezones, but it covers
                            // most and it's a guess anyway
                            mDialogMessage = res.getString(R.string.sync_log_clocks_unsynchronized, diff,
                    ...
                    {
                        mDialogMessage = res.getString(R.string.sync_log_error_fix, result[1] != null ? (" ("
                                + (String) result[1] + ")") : "");
                    ...
                    else
                    {
                        ...
                        {
                            mDialogMessage = res.getString(R.string.sync_log_error_specific, -1, result[0]);
                        }
                        else
                        {
                            mDialogMessage = res.getString(R.string.sync_generic_error);
    ...
    protected void onCreate(Bundle savedInstanceState) throws SQLException
    {
        ...
        // need to start this here in order to avoid showing deckpicker before
        // splashscreen
        ...
        // mStartedByBigWidget = intent.getIntExtra(EXTRA_START,
        // EXTRA_START_NOTHING);
        ...
        mDeckListAdapter = new SimpleAdapter(this, mDeckList, R.layout.deck_item, new String[] { "name", "new", "lrn",
                ...
                R.id.deckpicker_rev, // R.id.deckpicker_bar_mat,
                                     // R.id.deckpicker_bar_all,
        ...
        mDeckListAdapter.setViewBinder(new SimpleAdapter.ViewBinder()
        {
            ...
            public boolean setViewValue(View view, Object data, String text)
            {
                ...
                // } else if (view.getId() == R.id.deckpicker_bar_mat ||
                // view.getId() == R.id.deckpicker_bar_all) {
                // if (text.length() > 0 && !text.equals("-1.0")) {
                // View parent = (View)view.getParent().getParent();
                // if (text.equals("-2")) {
                // parent.setVisibility(View.GONE);
                // } else {
                // Utils.updateProgressBars(view, (int)
                // UIUtils.getDensityAdjustedValue(DeckPicker.this, 3.4f),
                // (int) (Double.parseDouble(text) *
                // ((View)view.getParent().getParent().getParent()).getHeight()));
                // if (parent.getVisibility() == View.INVISIBLE) {
                // parent.setVisibility(View.VISIBLE);
                // parent.setAnimation(ViewAnimation.fade(ViewAnimation.FADE_IN,
                // 500, 0));
                // }
                // }
                // }
                ...
                // view.setAnimation(ViewAnimation.fade(ViewAnimation.FADE_IN,
                // 500, 0));
                // return false;
                // }
    ...
    public void onRestoreInstanceState(Bundle savedInstanceState)
    {
        ...
        // mDeckList = (ArrayList<HashMap<String, String>>)
        // savedInstanceState.getSerializable("mDeckList");
    }
            ...
            case DIALOG_ERROR_HANDLING:
                ...
                builder.setOnCancelListener(new OnCancelListener()
                {
                    ...
                    public void onCancel(DialogInterface dialog)
                    {
                        if (mLoadFailed)
                        {
                            // dialog has been called because collection could
                            // not be opened
                            showDialog(DIALOG_LOAD_FAILED);
                        }
                        else
                        {
                            // dialog has been called because a db error
                            // happened
                                    ...
                                    public void onPostExecute(DeckTask.TaskData result)
                                    {
                                        ...
                                        if (result.getBoolean())
                                        {
                                            ...
                                            else
                                            {
                                                ...
                                                updateDecksList((TreeSet<Object[]>) info[0], (Integer) info[1],
                                                        (Integer) info[2]);
                                            }
                                        }
                                        else
