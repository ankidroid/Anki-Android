 ...
 */
public class DeckOptions extends PreferenceActivity implements
	...
	public class DeckPreferenceHack implements SharedPreferences {
		...
		protected void cacheValues() {
			...
			try {
				...
				mValues.put("showAnswerTimer", Boolean.toString(mOptions
						.getString("timer").equals("1")));
				mValues.put("autoPlayAudio",
						Boolean.toString(mOptions.getBoolean("autoplay")));
				// new
				JSONObject newOptions = mOptions.getJSONObject("new");
				mValues.put("newSteps",
						getDelays(newOptions.getJSONArray("delays")));
				mValues.put("newGradIvl", newOptions.getJSONArray("ints")
						.getString(0));
				mValues.put("newEasy", newOptions.getJSONArray("ints")
						.getString(1));
				mValues.put("newFactor", Integer.toString(newOptions
						.getInt("initialFactor") / 10));
				mValues.put("newOrder", newOptions.getString("order"));
				mValues.put("newPerDay", newOptions.getString("perDay"));
				mValues.put("newSeparate",
						Boolean.toString(newOptions.getBoolean("separate")));
				// rev
				...
				mValues.put("lapSteps",
						getDelays(lapOptions.getJSONArray("delays")));
				mValues.put("lapNewIvl", Integer.toString((int) (lapOptions
						.getDouble("mult") * 100)));
		...
		public class Editor implements SharedPreferences.Editor {
			...
			public boolean commit() {
				Log.d(AnkiDroidApp.TAG,
						"DeckOptions - commit() changes back to database");
				...
				try {
					for (Entry<String, Object> entry : mUpdate.valueSet()) {
						...
						if (entry.getKey().equals("name")) {
							...
							mOptions.getJSONObject("new").put("separate",
							...
							String steps = (String) entry.getValue();
							if (steps.matches("[0-9\\s]*")) {
								mOptions.getJSONObject("lapse").put("delays",
										getDelays(steps));
							}
						} else if (entry.getKey().equals("lapNewIvl")) {
							mOptions.getJSONObject("lapse")
									.put("mult",
											Float.parseFloat((String) entry
													.getValue()));
