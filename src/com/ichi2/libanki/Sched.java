...
public class Sched {
     ...
     */
    public void onClose() {
        mCol.getDb().getDatabase().execSQL("UPDATE cards SET queue = type WHERE queue BETWEEN -3 AND -2");
    }
...
//    }
    ...
    private TreeSet<Object[]>_groupChildren(ArrayList<Object[]> grps, boolean counts) {
    		...
    		if (lim != 0) {
                try {
                    ...
                    while (cur.moveToNext()) {
                        mNewQueue.add(new long[] { cur.getLong(0), cur.getInt(1) });
                    }
                } finally {
                    if (cur != null && !cur.isClosed()) {
                        cur.close();
...
// TODO: check order
    ...
    private boolean _fillLrn() {
        ...
        try {
            cur = mCol.getDb().getDatabase().rawQuery(
                    "SELECT due, id FROM cards WHERE did IN " + _deckLimit() + " AND queue = 1 AND due < " + mDayCutoff + " ORDER BY due LIMIT " + mReportLimit, null);
...
// TODO: check, if correct card is fetched
    ...
    private void _logLrn(Card card, int ease, JSONObject conf, boolean leaving, int type, int lastLeft) {
        ...
        log(card.getId(), mCol.getUsn(), ease, ivl, lastIvl, card.getFactor(), card.timeTaken(), type);
    }
    ...
    private void removeFailed(long[] ids) {
        ...
        mCol.getDb().getDatabase().execSQL(
                "UPDATE cards SET due = edue, queue = 2, mod = " + Utils.intNow() + ", usn = " + mCol.getUsn() + " WHERE queue = 1 AND type = 2 " + extra);
    }
...
// TODO: check correct order
    ...
    private void _logRev(Card card, int ease) {
        log(card.getId(), mCol.getUsn(), ease, card.getIvl(), card.getLastIvl(), card.getFactor(), card.timeTaken(), 1);
    }
    ...
    private int _ivlForFI(JSONObject conf, double ivl) {
    	...
    	try {
	    	...
	    	return (int) (ivl * Math.log(1 - (fi.getInt(0)/100.0)) / Math.log(1 - (fi.getInt(1)/100.0)));
    	} catch (JSONException e) {
	    	throw new RuntimeException(e);
    	}
    }
    ...
    private int _adjRevIvl(Card card, int idealIvl) {
        ...
        try {
            ...
            ArrayList<Integer> dues = mCol.getDb().queryColumn(Integer.class, "SELECT due FROM cards WHERE nid = "
                    + card.getNid() + " AND queue = 2 AND id != " + card.getId(), 0);
    ...
    private void _updateCutoff() {
        ...
        // update parents too
        for (JSONObject grp : mCol.getDecks().parents(mCol.getDecks().selected())) {
        	update(grp);
        }
    }
...
//    }
    ...
    public boolean revDue() {
    	return mCol.getDb().queryScalar("SELECT 1 FROM cards WHERE did IN " + _deckLimit() + " AND queue = 2 AND due <= " + mToday + " LIMIT 1", false) != 0;
    }


    /** true if there are any new cards due. */
    public boolean newDue() {
    	return mCol.getDb().queryScalar("SELECT 1 FROM cards WHERE did IN " + _deckLimit() + " AND queue = 0 LIMIT 1", false) != 0;
    }
    ...
    private void buryNote(long nid) {
        ...
        removeFailed(ids);
        mCol.getDb().getDatabase().execSQL("UPDATE cards SET queue = -2 WHERE nid = " + nid);
    }
