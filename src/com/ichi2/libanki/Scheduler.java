...
//    }
    ...
    private void _resetNewCount() {
    	...
    	try {
	    	...
	    	if (lim <= 0) {
	    		...
	    		mNewCount = (int) mDb.queryScalar("SELECT count() FROM (SELECT id FROM cards " +
	    				"WHERE queue = 0 " + _groupLimit() + " LIMIT " + lim + ")");
	    	}
		} catch (JSONException e) {
			throw new RuntimeException(e);
		}
    ...
    private void _resetNew() {
    	...
    	mNewQueue.clear();
    	try {
    		cur = mDb.getDatabase().rawQuery("SELECT id, due FROM cards WHERE queue = 0 " 
    				+ _groupLimit() + " ORDER BY due LIMIT " + lim, null);
            while (cur.moveToNext()) {
    ...
    private void _resetLrn() {
    	...
    	mLrnQueue.clear();
    	try {
    		cur = mDb.getDatabase().rawQuery("SELECT due, id FROM cards WHERE queue = 1 " 
    				+ _groupLimit() + " AND due < " + mDayCutoff + " ORDER BY due LIMIT " + mReportLimit, null);
            while (cur.moveToNext()) {
...
//    			ivl, lastIvl, factor, taken, type});    	
    ...
    private void removeFailed(int[] ids) {
    	...
    	mDb.getDatabase().execSQL("UPDATE cards SET " +
      		"due = edue, queue = 2, mod = " + Utils.intNow() +
    ...
    private void _resetRev() {
    	...
    	mRevQueue.clear();
    	try {
    		cur = mDb.getDatabase().rawQuery("SELECT id FROM cards WHERE queue = 2 " 
    				+ _groupLimit() + " AND due <= " + mToday + " ORDER BY " 
    ...
    private int _adjRevIvl(Card card, int idealIvl) {
		...
		try {
	    	...
	    	ArrayList<Integer> dues = mDb.queryColumn(Integer.class, "SELECT due FROM cards WHERE fid = " + card.getFId() + 
    ...
    private void suspendCards(int[] ids) {
    	removeFailed(ids);
    	mDb.getDatabase().execSQL("UPDATE cards SET queue = -1, mod = " + Utils.intNow() + 
    ...
    private void unsuspend(int[] ids) {
    	mDb.getDatabase().execSQL("UPDATE cards SET queue = type, mod = " + Utils.intNow() + 
