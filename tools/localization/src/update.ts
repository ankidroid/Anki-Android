    ...
    if (fileExt != ".csv") {
        for await (let line of rl) {
            ...
            if (line.startsWith("<?xml>")) {
                ...
                let regexp = new RegExp('%[0-9]\\s\\$|%[0-9]\\$\\s');
                if (!regexp.test(line)) {
                    errorOccured = true;
                }
            }
...
async function update(valuesDirectory: string, f: string, source: string, fileExt: string, isCrowdin: boolean, language = '') {
...
export async function updateI18nFiles() {
    for (let language of languages) {
        ...
        for (let f of fileNamesTmp) {
            ...
            let content = fs.readFileSync(temp_dir + "/" + language + "/" + f + fileExt).toString();
