...
class ContentProviderTest : InstrumentedTest() {
    ...
    fun testInsertField() {
        ...
        var noteType: NotetypeJson? = createBasicNoteType()
        val noteTypeId = noteType!!.getLong("id")
        val initialFieldsArr = noteType.getJSONArray("flds")
        ...
        assertNotNull("Check note type", noteType)
        val fldsArr = noteType!!.getJSONArray("flds")
        assertEquals(
            "Check fields length",
            (initialFieldCount + 1),
            fldsArr.length(),
    ...
    fun testInsertAndUpdateNoteType() {
        ...
        var col = reopenCol()
        try {
            var noteType: JSONObject? = col.notetypes.get(mid)
            assertNotNull("Check note type", noteType)
            ...
            val fields = noteType.getJSONArray("flds")
            ...
            // Test updating the note type CSS (to test updating MODELS_ID Uri)
            cv = ContentValues()
            ...
            // Update each of the templates in note type (to test updating MODELS_ID_TEMPLATES_ID Uri)
