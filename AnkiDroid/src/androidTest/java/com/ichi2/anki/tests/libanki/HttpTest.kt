...
class HttpTest {
    ...
    @Test
    fun testLogin(): Unit = runTest(testDispatcher) {
        ...
        // Tests for successful instantiation of Connection with no-argument constructor
        try {
        ...
        if (!Connection.Companion.isOnline) {
            Connection.login(testListener, invalidPayload, testDispatcher, testDispatcher)
            Assert.assertFalse(
