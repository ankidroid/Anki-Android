...
import com.ichi2.libanki.stats.Stats.SECONDS_PER_DAY
...
class SchedTest : RobolectricTest() {
    ...
    fun unburyWorksIfDeckIsNotSelected() {
        ...
        assertThat(
            "Card should be buried",
            getCardInDefaultDeck(sched),
            nullValue()
        )
        ...
        assertThat(
            "Card should no longer be buried",
            getCardInDefaultDeck(sched),
            notNullValue()
        )
    }
    ...
    fun learnCardsAreNotFiltered() {
        ...
        assertThat(
            "A learn card should not be moved into a dyn deck",
            dynamicDeck.lrnCount,
            `is`(0)
        )
        assertThat(
            "A learn card should not be moved into a dyn deck",
            dynamicDeck.newCount,
            `is`(0)
        )
        assertThat(
            "A learn card should not be moved into a dyn deck",
            dynamicDeck.revCount,
            `is`(0)
        )
    }
    ...
    fun testRevLogValues() {
        TimeManager.withMockInstance(
            MutableTime(
                MockTime.timeStamp(2020, 8, 4, 11, 22, 19, 123),
                10
            )
        ) { time: MutableTime ->
     ...
     */
    @get:Throws(ConfirmModSchemaException::class)
    private val colV1: Collection
        get() {
            val col = col
    ...
    fun test_reviewsV1() {
        ...
        assertThat(
            c.due,
            `is`(greaterThanOrEqualTo(TimeManager.time.intTime()))
        )
        ...
        assertEquals((20 * 60), col.sched.nextIvl(c, BUTTON_TWO))
    ...
    fun test_button_spacingV1() {
        ...
        assertEquals(
            "2 d",
            without_unicode_isolation(col.sched.nextIvlStr(targetContext, c, BUTTON_TWO))
        )
        assertEquals(
            "3 d",
    ...
    fun test_cram() {
        ...
        assertEquals((138 * 60 * 60 * 24), col.sched.nextIvl(c, BUTTON_TWO))
        ...
        assertEquals(60, col.sched.nextIvl(c, BUTTON_ONE))
        assertEquals(600, col.sched.nextIvl(c, BUTTON_TWO))
        assertEquals((138 * 60 * 60 * 24), col.sched.nextIvl(c, BUTTON_THREE))
        col.sched.answerCard(c, BUTTON_TWO)
    ...
    fun test_deckFlowV1() {
        ...
        note = col.newNote()
        note.setItem("Front", "two")
        note.model().put("did", addDeck("Default::2"))
        col.addNote(note)
        ...
        for (i in arrayOf("one", "two", "three")) {
            val c = card!!
