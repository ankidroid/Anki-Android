...
open class SchedV2Test : RobolectricTest() {
    ...
    fun test_bury() {
        ...
        col.reset()
        assertNull(card)
        col.sched.unburyCardsForDeck(type = BaseSched.UnburyType.MANUAL)
        c.load()
        Assert.assertEquals(QUEUE_TYPE_NEW, c.queue)
        c2.load()
        Assert.assertEquals(QUEUE_TYPE_SIBLING_BURIED, c2.queue)
        col.sched.unburyCardsForDeck(type = BaseSched.UnburyType.SIBLINGS)
        c2.load()
        Assert.assertEquals(QUEUE_TYPE_NEW, c2.queue)
        col.sched.buryCards(longArrayOf(c.id, c2.id))
        col.sched.unburyCardsForDeck(type = BaseSched.UnburyType.ALL)
        col.reset()
