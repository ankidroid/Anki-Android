...
class ModelTest : RobolectricTest() {
    ...
    fun test_text() {
        ...
        col.addNote(note)
        assertThat(note.cards(col)[0].q(), containsString("helloworld"))
    }
    ...
    fun test_cloze_mathjax() {
        ...
        assertThat(note.cards(col)[0].q(), containsString(clozeClass()))
        assertThat(note.cards(col)[1].q(), containsString(clozeClass()))
        assertThat(
            note.cards(col)[2].q(),
            not(containsString(clozeClass()))
        )
        assertThat(note.cards(col)[3].q(), containsString(clozeClass()))
        assertThat(note.cards(col)[4].q(), containsString(clozeClass()))

        note = col.newNote()
        note.setItem("Text", "\\(a\\) {{c1::b}} \\[ {{c1::c}} \\]")
        assertNotEquals(0, col.addNote(note))
        assertEquals(1, note.numberOfCards(col))
        val question = note.cards(col)[0].q()
        if (!BackendFactory.defaultLegacySchema) {
            // below needs updating to support latest backend output
            return
        }
    ...
    fun test_chained_mods() {
        ...
        assertThat(note.cards(col).get(0).a(), containsString("This <span class=cloze>phrase</span> demonstrates <span class=cloze>en chaine</span> clozes."
                                                    ));
