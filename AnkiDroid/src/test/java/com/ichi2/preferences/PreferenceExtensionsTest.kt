...
class PreferenceExtensionsTest {
    @Mock
    private val mMockPreferences: SharedPreferences? = null
    ...
    private fun getOrSetString(key: String, supplier: Supplier<String?>): String? {
        return getOrSetString(mMockPreferences!!, key, supplier)
    }
    ...
    fun setUp() {
        ...
        Mockito.`when`(mMockPreferences.getString(ArgumentMatchers.eq(VALID_KEY), ArgumentMatchers.anyString())).thenReturn(VALID_RESULT)
        Mockito.`when`(mMockPreferences.edit()).thenReturn(mMockEditor)
        Mockito.`when`(mMockEditor!!.putString(ArgumentMatchers.anyString(), ArgumentMatchers.anyString())).thenReturn(mMockEditor)
    }
    ...
    fun existingKeyReturnsMappedValue() {
        ...
        Assert.assertEquals(ret, VALID_RESULT)
    }

    @Test
    fun missingKeyReturnsLambdaValue() {
        val ret = forMissingKey
        Assert.assertEquals(ret, LAMBDA_RETURN)
    }
    ...
    fun noLambdaExceptionIfKeyExists() {
        AnkiAssert.assertDoesNotThrow { getOrSetString(VALID_KEY, EXCEPTION_SUPPLIER) }
    }
