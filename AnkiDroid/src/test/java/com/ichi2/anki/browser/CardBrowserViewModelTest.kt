...
class CardBrowserViewModelTest : JvmTest() {
    ...
    fun `change deck in notes mode 15444`() =
        runViewModelTest {
            ...
            for (i in 0 until 5) {
                addNoteUsingBasicAndReversedNoteType()
            }
    ...
    fun filterByFlagDisplaysProperly() =
        runViewModelTest {
            val cardWithRedFlag = addNoteUsingBasicNoteType("Card with red flag", "Reverse")
            flagCardForNote(cardWithRedFlag, Flag.RED)

            val cardWithGreenFlag = addNoteUsingBasicNoteType("Card with green flag", "Reverse")
            flagCardForNote(cardWithGreenFlag, Flag.GREEN)

            val anotherCardWithRedFlag = addNoteUsingBasicNoteType("Second card with red flag", "Reverse")
    ...
    fun `selected card and note ids`() {
        val notes = List(2) { addNoteUsingBasicAndReversedNoteType() }
    ...
    fun `notes - search for marked`() =
        runTest {
            addNoteUsingBasicAndReversedNoteType("hello", "world").also { note ->
                NoteService.toggleMark(note)
            }
            addNoteUsingBasicAndReversedNoteType("hello2", "world")
