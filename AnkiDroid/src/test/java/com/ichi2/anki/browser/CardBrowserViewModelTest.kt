...
class CardBrowserViewModelTest : JvmTest() {
    ...
    fun filterByFlagDisplaysProperly() =
        runViewModelTest {
            val cardWithRedFlag = addBasicNoteType("Card with red flag", "Reverse")
            flagCardForNote(cardWithRedFlag, Flag.RED)

            val cardWithGreenFlag = addBasicNoteType("Card with green flag", "Reverse")
            flagCardForNote(cardWithGreenFlag, Flag.GREEN)

            val anotherCardWithRedFlag = addBasicNoteType("Second card with red flag", "Reverse")
    ...
    fun `selected card and note ids`() {
        val notes = List(2) { addBasicAndReversedNoteType() }
    ...
    fun `notes - search for marked`() =
        runTest {
            addBasicAndReversedNoteType("hello", "world").also { note ->
                NoteService.toggleMark(note)
            }
            addBasicAndReversedNoteType("hello2", "world")
