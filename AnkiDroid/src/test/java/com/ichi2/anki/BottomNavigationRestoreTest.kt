...
class BottomNavigationRestoreTest : RobolectricTest() {
    ...
    @Test
    fun `restoring after process death on Home survives a previously opened browser`() =
    ...
    @Test
    fun `restoring after process death survives a browser opened before the bottom navigation was turned off`() =
        withBottomNavigation {
            val controller = launchOn(NavigationItem.BROWSER)
