...
class CardBrowserTest : RobolectricTest() {
    ...
    fun flagValueIsShownOnCard() {
        val n = addNoteUsingBasicNoteType("1", "back")
    ...
    fun tagWithBracketsDisplaysProperly() =
        runTest {
            val n = addNoteUsingBasicNoteType("Hello", "World")
            n.addTag("sketchy::(1)")
            n.flush()
    ...
    fun previewWorksAfterSort() =
        runTest {
            // #7286
            val cid1 = addNoteUsingBasicNoteType("Hello", "World").cards()[0].id
            val cid2 = addNoteUsingBasicNoteType("Hello2", "World2").cards()[0].id
    ...
    fun resetDataTest() =
        runTest {
            TimeManager.reset()
            addNoteUsingBasicNoteType("Hello", "World").firstCard().update {
                due = 5
    ...
    fun checkSearchString() =
        runTest {
            addNoteUsingBasicNoteType("Hello", "John")
            addNoteUsingBasicNoteType("New", "world").firstCard().update {
    ...
    fun checkIfSearchAllDecksWorks() =
        runTest {
            ...
            addNoteUsingBasicNoteType("Front", "Back").firstCard().update {
    ...
    fun `'notes-only mode' returns one card from each note`() =
        runTest {
            ...
            addNoteUsingBasicAndReversedNoteType("Hello", "World")
            addNoteUsingBasicAndReversedNoteType("Hello", "Anki")
    ...
    private fun selectOneOfManyCards(
        ...
        val listener =
            toSelect.onItemLongClickListener
    ...
    private fun getBrowserWithNotes(
        ...
        if (reversed) {
            for (i in 0 until noteCount) {
                addNoteUsingBasicAndReversedNoteType(i.toString(), "back")
            }
        } else {
            for (i in 0 until noteCount) {
                addNoteUsingBasicNoteType(i.toString(), "back")
            }
        }
    ...
    fun `deck id is remembered - issue 15072`() =
        runTest {
            ...
            addNoteUsingBasicNoteType().moveToDeck("First")
            addNoteUsingBasicNoteType().moveToDeck("Second")
    ...
    fun nextDueTest() {
        TimeManager.reset()
        val n = addNoteUsingBasicNoteType("Front", "Back")
        val c = n.firstCard()
        val decks = col.decks
        val cal = Calendar.getInstance()
        val expectedDate = LanguageUtil.getShortDateFormatFromMs(cal.timeInMillis)
    ...
    fun `tts tags are stripped`() {
        ...
        val question =
            CardCache(note.firstCard().id, col, 1, CARDS)
