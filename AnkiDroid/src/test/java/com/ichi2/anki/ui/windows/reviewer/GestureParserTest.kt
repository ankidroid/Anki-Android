...
class GestureParserTest {
    ...
    // JSON Parsing Tests
    @Test
    fun `parses tapOrSwipe from JSON bytes`() {
        val jsonString = """{"x":10,"y":20,"deltaX":30,"deltaY":40,"time":123456,"scrollDirection":"h"}"""
        val parsed = RawGesture.TapOrSwipe(jsonString.toByteArray())

        assertEquals(10, parsed.x)
        assertEquals(20, parsed.y)
        assertEquals(30, parsed.deltaX)
        assertEquals(40, parsed.deltaY)
        assertEquals(123456L, parsed.time)
        assertEquals("h", parsed.scrollDirection)
    }

    @Test
    fun `parses tapOrSwipe from JSON bytes without scrollDirection`() {
        val jsonString = """{"x":10,"y":20,"deltaX":30,"deltaY":40,"time":123456}"""
        val parsed = RawGesture.TapOrSwipe(jsonString.toByteArray())

        assertNull(parsed.scrollDirection)
    }

    @Test
    fun `parses multi-touch from JSON bytes`() {
        val jsonString = """{"touchCount":2}"""
        val parsed = RawGesture.MultiTouch(jsonString.toByteArray())

        assertEquals(2, parsed.touchCount)
    }
    //endregion
