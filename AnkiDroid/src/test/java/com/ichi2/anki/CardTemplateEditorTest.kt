...
class CardTemplateEditorTest : RobolectricTest() {
     ...
     */
    @Test
    fun testDeleteTemplateWithSelectivelyGeneratedCards() = runTest {
     ...
     */
    @Test
    fun testDeletePendingAddExistingCardCount() = runTest {
        val modelName = "Basic (optional reversed card)"
        val collectionBasicModelOriginal = getCurrentDatabaseModelCopy(modelName)
    ...
    private fun addCardType(testEditor: CardTemplateEditor, shadowTestEditor: ShadowActivity) {
        ...
        val numAffectedCards = if (!testEditor.tempModel.isOrdinalPendingAdd(ordinal)) {
...
""".trim(),
    ...
    private suspend fun NotetypeJson.getCardIds(vararg ords: Int): List<Long>? =
        withCol { notetypes.getCardIdsForModel(id, ords) }
}
