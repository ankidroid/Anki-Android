...
class BottomNavigationAnalyticsTest : RobolectricTest() {
    ...
    fun `each tab is reported under its screen name`() =
        withBottomNavigation {
            mapOf(
                ...
                assertThat(screenViewsDuring { tap(tab) }, equalTo(listOf(screenName)))
            }
        }

    @Test
    fun `tapping the open tab again is not reported`() =
        withBottomNavigation {
            tap(NavigationItem.BROWSER)

            assertThat(screenViewsDuring { tap(NavigationItem.BROWSER) }, empty())
        }
    ...
    private fun ActivityController<DeckPicker>.tap(tab: NavigationItem) {
        get()
            .findViewById<BottomNavigationView>(R.id.bottom_navigation)
            .findViewById<View>(tab.id)
            .performClick()
    }
}
