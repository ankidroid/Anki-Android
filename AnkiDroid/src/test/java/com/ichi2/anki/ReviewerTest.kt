...
class ReviewerTest : RobolectricTest() {
    ...
    fun noErrorShouldOccurIfSoundFileNotPresent() {
        val firstNote = addNoteUsingBasicNoteType("[[sound:not_on_file_system.mp3]]", "World")
        ...
        runTest {
            ...
            val cards =
                arrayOf(
                    ...
                    addNoteUsingBasicNoteType("2", "bar").firstCard(),
                    addNoteUsingBasicNoteType("3", "bar").firstCard(),
                )
            waitForAsyncTasksToComplete()
    ...
    private fun runReviewer(
        ...
        for (frontSide in cards) {
            addNoteUsingBasicNoteType(front = frontSide)
        }
