...
//    }
    ...
    fun verifyStartupAndCloseWithNoCollectionDoesNotCrash() {
        ...
        ActivityScenario.launchActivityForResult<NoteEditor>(intent).use { scenario ->
    ...
    fun ActivityScenario<NoteEditor>.onNoteEditor(block: (NoteEditorFragment) -> Unit) {
        val wrapped = AtomicReference<Throwable?>(null)
        this.onActivity { activity: NoteEditor ->
            try {
    ...
    @DuplicatedCode("NoteEditor in androidTest")
    fun NoteEditor.getEditor(): NoteEditorFragment =
