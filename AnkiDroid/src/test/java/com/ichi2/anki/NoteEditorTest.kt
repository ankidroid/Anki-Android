...
//    }
    ...
    fun copyNoteCopiesDeckId() {
        ...
        val n = super.addNoteUsingBasicNoteType("Test", "Note")
    ...
    fun `edit note in filtered deck from reviewer - 15919`() {
        ...
        val note = addNoteUsingBasicNoteType().updateCards { did = homeDeckId }
    ...
    private fun makeNoteForType(noteType: NoteType): NotetypeJson? =
        when (noteType) {
            ...
            NoteType.BACK_TO_FRONT -> {
                val name = super.addNonClozeNoteType("Reversed", arrayOf("Front", "Back"), "{{Back}}", "{{Front}}")
                col.notetypes.byName(name)
            }
            NoteType.THREE_FIELD_INVALID_TEMPLATE -> {
                val name = super.addNonClozeNoteType("Invalid", arrayOf("Front", "Back", "Side"), "", "")
                col.notetypes.byName(name)
            }
    ...
    private fun getNoteEditorEditingExistingBasicNote(
        ...
        val n = super.addNoteUsingBasicNoteType(front, back)
