...
//    }
    ...
    fun verifyStartupAndCloseWithNoCollectionDoesNotCrash() {
        ...
        val intent = NoteEditor.getIntent(targetContext, NoteEditor.OpenNoteEditorDestination.AddNote())
    ...
    fun copyNoteCopiesDeckId() {
        ...
        val copyNoteBundle = NoteEditor.OpenNoteEditorDestination.CopyNote(editor.deckId, editor.fieldsText)
        ...
        assertThat("Deck ID in the intent should be the selected deck id", copyNoteBundle.toBundle().getLong(NoteEditor.EXTRA_DID, -404L), equalTo(currentDid))
    ...
    fun processTextIntentShouldCopyFirstField() {
        ...
        val bundle = NoteEditor.OpenNoteEditorDestination.ProcessText("hello\nworld")
    ...
    private fun getNoteEditorAddingNote(from: FromScreen): NoteEditor {
        ...
        val bundle =
            when (from) {
                REVIEWER -> NoteEditor.OpenNoteEditorDestination.AddNoteFromReviewer()
                DECK_LIST -> NoteEditor.OpenNoteEditorDestination.AddNote()
            }
    ...
    private fun getNoteEditorEditingExistingBasicNote(n: Note, from: FromScreen): NoteEditor {
        val bundle =
            when (from) {
                REVIEWER -> NoteEditor.OpenNoteEditorDestination.EditCard(n.firstCard().id, DEFAULT)
                DECK_LIST -> NoteEditor.OpenNoteEditorDestination.AddNote()
            }
        return openNoteEditorWithArgs(bundle)
    }
