...
value class AppWidgetIds(
    ...
    override fun iterator(): Iterator<AppWidgetId> =
        object : Iterator<AppWidgetId> {
            var next = 0

            override fun next(): AppWidgetId = AppWidgetId(ids[next++])

            override fun hasNext(): Boolean = next < ids.size
        }

    companion object {
        fun of(ids: IntArray?) = ids?.let { AppWidgetIds(it) }

        fun of(ids: IntArray) = AppWidgetIds(ids)
    }
}
