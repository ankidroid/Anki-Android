...
open class CardBrowser :
    ...
    private fun setupFlows() {
        ...
        viewModel.flowOfFocusedRow.launchCollectionInLifecycleScope { onFocusedRowChanged(it) }
    }
                ...
                if (fragmented && !isDeckEmpty) {
                    ...
                    if (reviewerCardId != -1L) {
                        ...
                        if (cardOrNoteId != null) {
                            viewModel.focusedRow = cardOrNoteId
                        }
                    } else if (viewModel.focusedRow == null && viewModel.cards.isNotEmpty()) {
                        // Otherwise, set the first row as focused if none is set
                        viewModel.focusedRow = viewModel.cards[0]
                    }
