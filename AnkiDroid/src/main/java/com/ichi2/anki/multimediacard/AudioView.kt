    ...
    fun notifyStopRecord() {
        if (status == Status.RECORDING) {
            try {
                ...
                showSnackbar(gtxt(R.string.multimedia_editor_audio_view_recording_failed), Snackbar.LENGTH_SHORT)
            }
            status = Status.IDLE
            ...
            override fun onClick(v: View) {
                ...
                when (status) {
                    Status.IDLE -> try {
                        ...
                        Timber.e(e)
                        showSnackbar(gtxt(R.string.multimedia_editor_audio_view_playing_failed), Snackbar.LENGTH_SHORT)
                        status = Status.IDLE
                    }
                    Status.PAUSED -> {
        ...
        private val mOnClickListener: OnClickListener = object : OnClickListener {
            override fun onClick(v: View) {
                ...
                if (!canRecordAudio(mContext)) {
                    ...
                    showSnackbar(resources.getString(R.string.multimedia_editor_audio_permission_denied), Snackbar.LENGTH_SHORT)
                    return
                }
                when (status) {
                    Status.IDLE, Status.STOPPED -> {
