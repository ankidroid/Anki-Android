...
class ModelFieldEditor :
    ...
    /**
     * Displays a repositioning dialog allowing the user to move a field to a new position.
     */
    private fun repositionField() {
        showRepositionDialog(fieldsLabels.size) { newPosition ->
            if (newPosition == currentPos + 1) return@showRepositionDialog
    ...
    fun handleAction(contextMenuAction: ModelEditorContextMenuAction) {
        when (contextMenuAction) {
            ...
            ModelEditorContextMenuAction.Reposition -> repositionField()
