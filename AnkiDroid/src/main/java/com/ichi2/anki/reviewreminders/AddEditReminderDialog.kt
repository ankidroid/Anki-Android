...
class AddEditReminderDialog : DialogFragment() {
    ...
    fun setUpDialogFields() {
        ...
        // 1. viewModel -> state: Define how the viewModel's stored state changes as the user edits the fields of the dialog
        cardThresholdInput.doOnTextChanged { text, _, _, _ ->
            // We interpret invalid values, which should be limited to just the empty string due to the XML element's settings, as 0
