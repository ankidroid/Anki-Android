...
class FilteredDeckOptionsFragment : Fragment(R.layout.fragment_filtered_deck_options) {
                ...
                viewModel.state.collect { state ->
                    when (state) {
                        is Initializing -> {
                            if (state.throwable != null) {
                                ...
                                AlertDialog.Builder(requireActivity()).show {
                                    setTitle(CommonString.import_title_error)
                                    message(text = state.throwable.toString())
                                    cancelable(false)
                                    positiveButton(CommonString.dialog_exit) { requireActivity().finish() }
                                }
                            } else {
                        ...
                        is FilteredDeckOptions -> {
                            ...
                            if (state.throwable != null) {
                                AlertDialog.Builder(requireActivity()).show {
                                    title(CommonString.import_title_error)
    ...
    private fun bindState(state: FilteredDeckOptions) {
        ...
        binding.deckNameInputLayout.error =
            when (state.nameInputError) {
                FilteredNameInputError.Empty -> getString(CommonString.empty_cram_label)
                FilteredNameInputError.AlreadyExists -> getString(CommonString.error_name_exists)
                null -> null
            }
        ...
        binding.filterLimitInputLayout.error =
            when (state.filter1State.error) {
                SearchInputError.Empty -> getString(CommonString.empty_cram_label)
    ...
    private fun bindLabels() {
        ...
        binding.rescheduleDelayAgainLayout.hint = getString(CommonString.filtered_option_delay_again)
        binding.rescheduleDelayHardLayout.hint = getString(CommonString.filtered_option_delay_hard)
        binding.rescheduleDelayGoodLayout.hint = getString(CommonString.filtered_option_delay_good)
    }
