...
class FilteredDeckOptionsFragment : Fragment(R.layout.fragment_filtered_deck_options) {
    ...
    private fun bindListeners() {
        binding.deckNameInput.doOnTextChanged { text, _, _, _ ->
            if (text == null) return@doOnTextChanged
            viewModel.onDeckNameChange(text.toString())
        }
        // filter#1
        binding.filterSearchInput.doOnTextChanged { text, _, _, _ ->
            if (text == null) return@doOnTextChanged
            viewModel.onSearchChange(FilterIndex.First, text.toString())
        }
        binding.filterLimitInput.doOnTextChanged { text, _, _, _ ->
            if (text == null) return@doOnTextChanged
            viewModel.onLimitChange(FilterIndex.First, text.toString())
        }
        ...
        // filter#2
        binding.switchSecondFilter.setOnCheckedChangeListener { _, isChecked ->
            viewModel.onSecondFilterStatusChange(isChecked)
        }
        binding.secondFilterSearchInput.doOnTextChanged { text, _, _, _ ->
            if (text == null) return@doOnTextChanged
            viewModel.onSearchChange(FilterIndex.Second, text.toString())
        }
        binding.secondFilterLimitInput.doOnTextChanged { text, _, _, _ ->
            if (text == null) return@doOnTextChanged
            viewModel.onLimitChange(FilterIndex.Second, text.toString())
        }
        ...
        // reschedule
        binding.checkBoxReschedule.setOnCheckedChangeListener { _, isChecked ->
            viewModel.onRescheduleChange(isChecked)
        }
        binding.rescheduleDelayAgainInput.doOnTextChanged { text, _, _, _ ->
            if (text == null) return@doOnTextChanged
            viewModel.onRescheduleDelayChange(RescheduleDelay.Again, text.toString())
        }
        binding.rescheduleDelayHardInput.doOnTextChanged { text, _, _, _ ->
            if (text == null) return@doOnTextChanged
            viewModel.onRescheduleDelayChange(RescheduleDelay.Hard, text.toString())
        }
        binding.rescheduleDelayGoodInput.doOnTextChanged { text, _, _, _ ->
            if (text == null) return@doOnTextChanged
            viewModel.onRescheduleDelayChange(RescheduleDelay.Good, text.toString())
        }
        // other buttons
        binding.checkBoxAllowEmpty.setOnCheckedChangeListener { _, isChecked ->
            viewModel.onAllowEmptyChange(isChecked)
        }
