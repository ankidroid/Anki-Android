 ...
 */
class SyncHandler() : SyncErrorDialogListener {
    private lateinit var activity: AnkiActivity
    private lateinit var delegate: SyncHandlerDelegate

    /**
     * Initialize the handler
     */
    fun lateInit(activity: AnkiActivity, delegate: SyncHandlerDelegate) {
        this.activity = activity
        this.delegate = delegate
        loginForSyncLauncher = activity.registerForActivityResult(
        ...
        notificationPermissionLauncher = activity.registerForActivityResult(ActivityResultContracts.RequestPermission()) {
     ...
     */
    var syncMenuState: SyncMenuState? = null

    private lateinit var loginForSyncLauncher: ActivityResultLauncher<Intent>
    ...
    fun setupMediaSyncMenuItem(menu: Menu) {
        ...
        syncMediaProgressJob = activity.lifecycleScope.launch {
            flow.flowWithLifecycle(activity.lifecycle).collectLatest {
                ...
                if (workInfo?.state == WorkInfo.State.RUNNING && progressIndicator?.isVisible == false) {
                    Timber.i("SyncHandler: Showing media sync progress indicator")
                    progressIndicator.isVisible = true
                } else if (progressIndicator?.isVisible == true) {
                    Timber.i("SyncHandler: Hiding media sync progress indicator")
    ...
    data class SyncMenuState(val syncIcon: SyncIconState)
    ...
    private suspend fun handleNormalSync(
        ...
        var auth2 = auth
        val output = activity.withProgress(
            extractProgress = {
        ...
        if (output.hasNewEndpoint()) {
            ...
            auth2 = anki.sync.syncAuth { this.hkey = auth.hkey; endpoint = output.newEndpoint }
        }
