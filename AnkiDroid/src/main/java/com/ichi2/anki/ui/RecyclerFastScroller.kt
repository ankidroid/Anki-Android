        ...
        private val scrollTask =
            Runnable {
                ...
                try {
                    ...
                    val targetIndexDouble = pendingScrollProportion * adapter.itemCount
                    val targetIndex = targetIndexDouble.toInt().coerceIn(0, adapter.itemCount - 1)

                    // The fraction is how deep into the card we are (0.0 to 1.0)
                    val fraction = targetIndexDouble - targetIndex

                    // Estimate height using the first visible view, this is a heuristic
                    val firstChild = recyclerView?.getChildAt(0)
                    val estimatedHeight = firstChild?.height ?: 0
                    ...
                    (lm as? LinearLayoutManager)?.scrollToPositionWithOffset(targetIndex, offset)
                } catch (e: Exception) {
        ...
        override fun onTouchEvent(event: MotionEvent): Boolean {
            return when (event.actionMasked) {
                MotionEvent.ACTION_DOWN, MotionEvent.ACTION_MOVE -> {
                    ...
                    recyclerView?.postDelayed(scrollTask, 20)
                    true
                }
