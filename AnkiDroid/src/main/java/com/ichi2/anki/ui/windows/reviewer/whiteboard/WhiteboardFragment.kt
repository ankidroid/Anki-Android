...
class WhiteboardFragment :
    ...
    /**
     * Rebuilds the brush toolbar using the new BrushButton component.
     */
    private fun updateBrushToolbar(brushesInfo: List<BrushInfo>) {
        ...
        brushesInfo.forEachIndexed { index, brush ->
            // add the brush in both horizontal and vertical layouts
    ...
    private fun configureBrushButton(
        ...
        // Get the icon, make it mutable, and set the fill color
    ...
    private fun showStrokeWidthPopup(
        ...
        val strokeSlider = popupView.findViewById<Slider>(R.id.stroke_width_slider_popup)
        val colorButton = popupView.findViewById<ImageButton>(R.id.color_picker_button)

        val currentBrush = viewModel.brushes.value.getOrNull(brushIndex) ?: return

        // Set the color of the brush preview
        val background = colorButton.background as LayerDrawable
        val fillDrawable = background.findDrawableByLayerId(R.id.brush_preview_fill) as GradientDrawable
    ...
    private fun showEraserOptionsPopup(anchorView: View) {
        ...
        // Initialize slider with the correct width for the current mode
        ...
        when (viewModel.eraserMode.value) {
            EraserMode.PATH -> toggleGroup.check(R.id.eraser_mode_stroke)
            EraserMode.PIXEL -> toggleGroup.check(R.id.eraser_mode_ink)
        }
        toggleGroup.addOnButtonCheckedListener { _, checkedId, isChecked ->
            if (isChecked) {
                when (checkedId) {
                    R.id.eraser_mode_stroke -> {
                        viewModel.setEraserMode(EraserMode.PATH)
                        slider.value = viewModel.strokeEraserStrokeWidth.value
                    }
                    R.id.eraser_mode_ink -> {
                        viewModel.setEraserMode(EraserMode.PIXEL)
        ...
        eraserPopup = PopupWindow(popupView, ViewGroup.LayoutParams.WRAP_CONTENT, ViewGroup.LayoutParams.WRAP_CONTENT, true)
    ...
    private fun updateLayoutForAlignment(alignment: ToolbarAlignment) {
        ...
        if (isVertical) {
            ...
            val margin = (8 * resources.displayMetrics.density).roundToInt()
            dividerParams.setMargins(0, margin, 0, margin)
        } else {
