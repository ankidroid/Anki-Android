 ...
 * whiteboard settings from SharedPreferences. This isolates data handling from the ViewModel.
 */
class WhiteboardRepository(
    ...
    var eraserMode: String
        get() = sharedPreferences.getString(KEY_ERASER_MODE, EraserMode.PIXEL.name) ?: EraserMode.PIXEL.name
        set(value) = sharedPreferences.edit { putString(KEY_ERASER_MODE, value) }
    ...
    var toolbarAlignment: String
        get() = sharedPreferences.getString(KEY_TOOLBAR_ALIGNMENT, ToolbarAlignment.BOTTOM.name) ?: ToolbarAlignment.BOTTOM.name
        set(value) = sharedPreferences.edit { putString(KEY_TOOLBAR_ALIGNMENT, value) }
        ...
        fun getDefaultBrushes(isDarkMode: Boolean): List<BrushInfo> =
            if (isDarkMode) {
                listOf(
                    ...
                    BrushInfo(color = Color.CYAN, width = DEFAULT_STROKE_WIDTH),
                    BrushInfo(color = Color.MAGENTA, width = DEFAULT_STROKE_WIDTH),
                )
            } else {
                listOf(
                    BrushInfo(color = Color.BLACK, width = DEFAULT_STROKE_WIDTH),
                    BrushInfo(color = Color.RED, width = DEFAULT_STROKE_WIDTH),
                    BrushInfo(color = Color.BLUE, width = DEFAULT_STROKE_WIDTH),
                )
            }
    }
}
