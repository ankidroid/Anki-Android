 ...
 *
 * This feature first became available in API 31. Hence, we do not show legacy storage permissions (which were available
...
class AllPermissionsExplanationFragment : PermissionsFragment(R.layout.all_permissions_explanation_fragment) {
    val externalStoragePermission: PermissionsItem? by lazy { view?.findViewById(R.id.manage_external_storage_permission_item) }
    val notificationPermission: PermissionsItem? by lazy { view?.findViewById(R.id.post_notification_permission_item) }
    val recordAudioPermission: PermissionsItem? by lazy { view?.findViewById(R.id.record_audio_permission_item) }

    /**
     * Attempts to open the dialog for granting permissions. Falls back to opening the OS settings if the dialog fails to
     * show up or if the permission is rejected by the user. The dialog may fail to show up if the user has previously denied the
     * permissions multiple times, if the user selects "don't ask again" on the notification dialog, etc.
     */
    ...
    override fun onViewCreated(
        ...
        // The audio permission requires a minimum API level of API 23, which is less than AnkiDroid's
        // minimum targeted API version, so we show this permission unconditionally.
     ...
     */
    private fun PermissionsItem.offerToGrantOrRevokeOnClick(associatedPermission: String) {
