...
class DialogHandler(activity: AnkiActivity) : Handler() {
    val mActivity: WeakReference<AnkiActivity>
    override fun handleMessage(msg: Message) {
        ...
        if (msg.what == MSG_SHOW_COLLECTION_LOADING_ERROR_DIALOG) {
            ...
            msgData.getString("importPath")?.let { (mActivity.get() as DeckPicker?)?.showImportDialog(ImportDialog.DIALOG_IMPORT_REPLACE_CONFIRM, it) }
        } else if (msg.what == MSG_SHOW_COLLECTION_IMPORT_ADD_DIALOG) {
            // Handle import of deck package APKG
            msgData.getString("importPath")?.let { (mActivity.get() as DeckPicker?)?.showImportDialog(ImportDialog.DIALOG_IMPORT_ADD_CONFIRM, it) }
        } else if (msg.what == MSG_SHOW_SYNC_ERROR_DIALOG) {
            val id = msgData.getInt("dialogType")
            val message = msgData.getString("dialogMessage")
            (mActivity.get() as DeckPicker?)?.showSyncErrorDialog(id, message.toString())
        } else if (msg.what == MSG_SHOW_EXPORT_COMPLETE_DIALOG) {
            // Export complete
            val f: AsyncDialogFragment = DeckPickerExportCompleteDialog.newInstance(msgData.getString("exportPath"))
            ...
            if (id != MediaCheckDialog.DIALOG_CONFIRM_MEDIA_CHECK) {
                ...
                (mActivity.get() as DeckPicker?)?.showMediaCheckDialog(id, checkList.toList() as List<List<String>>)
            }
            ...
            val preferences: SharedPreferences = AnkiDroidApp.getSharedPrefs(mActivity.get())
            val res: Resources = mActivity.get()?.resources!!
            val col: Collection = mActivity.get()?.getCol()!!
            ...
            if (!limited && hkey!!.length > 0 && Connection.isOnline()) {
                ...
                if (limited) {
                    val remainingTimeInSeconds = Math.max((INTENT_SYNC_MIN_INTERVAL - millisecondsSinceLastSync) / 1000, 1)
                    // getQuantityString needs an int
                    val remaining = Math.min(Int.MAX_VALUE.toLong(), remainingTimeInSeconds).toInt()
    ...
    init {
        ...
        mActivity = WeakReference<AnkiActivity>(activity)
    }
}
