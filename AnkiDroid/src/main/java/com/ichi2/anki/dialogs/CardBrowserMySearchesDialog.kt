...
class CardBrowserMySearchesDialog : AnalyticsDialogFragment() {
    ...
    override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
        ...
        val type = CardBrowserMySearches.fromCode(requireArguments().getInt(CARD_BROWSER_MY_SEARCHES_SAVED_TYPE_KEY))
        if (type == CardBrowserMySearches.List) {
            savedFilters = requireArguments().getSerializableCompat(CARD_BROWSER_MY_SEARCHES_SAVED_FILTER_KEY)
        ...
        } else if (type == CardBrowserMySearches.Save) {
            val currentSearchTerms = requireArguments().getString(CARD_BROWSER_MY_SEARCHES_SAVED_CURRENT_SEARCH_TERMS_KEY)
            return dialog
                .show {
    ...
    enum class CardBrowserMySearches(
        ...
        companion object {
            fun fromCode(c: Int) = CardBrowserMySearches.entries.first { it.code == c }
        }
    }

    companion object {
        const val CARD_BROWSER_MY_SEARCHES_SAVED_FILTER_KEY = "savedFilters"
        const val CARD_BROWSER_MY_SEARCHES_SAVED_TYPE_KEY = "type"
        const val CARD_BROWSER_MY_SEARCHES_SAVED_CURRENT_SEARCH_TERMS_KEY = "currentSearchTerms"
        ...
        fun newInstance(
            ...
            val args = Bundle()
            args.putSerializable(CARD_BROWSER_MY_SEARCHES_SAVED_FILTER_KEY, savedFilters?.let(::HashMap))
            args.putInt(CARD_BROWSER_MY_SEARCHES_SAVED_TYPE_KEY, type.code)
            args.putString(CARD_BROWSER_MY_SEARCHES_SAVED_CURRENT_SEARCH_TERMS_KEY, currentSearchTerms)
