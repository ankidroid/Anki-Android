...
open class AnkiProgressDialogFragment : DialogFragment() {
    ...
    private fun setupViewModelFromArguments() {
        ...
        arguments?.let { args ->
            ...
            val cancelButtonText =
                if (args.containsKey(ARG_CANCEL_BUTTON_TEXT)) {
                    args.getString(ARG_CANCEL_BUTTON_TEXT)
                } else {
                    null
                }
    ...
    private fun updateUiFromViewModel() {
        try {
            ...
            Timber.e(e, "Error in updateUiFromViewModel")
        }
    }
    ...
    private fun updateMessageUi(message: String) {
        ...
        progressMessageView?.let {
            it.text = message
            Timber.d("UI message set to: '%s'", message)
        } ?: Timber.e("progressMessageView is null during message update")
    }

    private fun updateProgressUi(progress: AnkiProgressDialogViewModel.Progress?) {
        val isIndeterminate = progress == null
        Timber.d("Setting indeterminate state: %s", isIndeterminate)
    ...
    private fun updateCancelableState(cancelable: Boolean) {
        Timber.d("Setting cancelable state: %s", cancelable)
            ...
            viewLifecycleOwner.repeatOnLifecycle(Lifecycle.State.CREATED) {
                launch {
                    viewModel.message.collectLatest { message ->
                        Timber.d("Flow received new message: '%s'", message)
                ...
                launch {
                    viewModel.progress.collect { progress ->
                        Timber.d("Flow received new progress: %s", progress)
                ...
                launch {
                    viewModel.cancelableViaBackButton.collectLatest { cancelable ->
                        Timber.d("Flow received new cancelable state: %s", cancelable)
    ...
    private fun updateProgressCounter() {
        ...
        Timber.d(
            "Progress counter updated to: %d/%d",
    ...
    fun updateMessage(newMessage: String) {
        Timber.d("updateMessage called with: '%s'", newMessage)
            ...
            progressMessageView?.let { textView ->
                if (isAdded && !isDetached) {
                    activity?.runOnUiThread {
                        ...
                        Timber.d("Direct UI update with message: '%s'", newMessage)
                    }
                }
            }
        } catch (e: Exception) {
            Timber.e(e, "Exception while updating message in progress dialog")
        }
    }
    ...
    fun updateProgress(
        ...
        try {
            ...
            Timber.e(e, "Exception while updating progress in progress dialog")
        }
    }
