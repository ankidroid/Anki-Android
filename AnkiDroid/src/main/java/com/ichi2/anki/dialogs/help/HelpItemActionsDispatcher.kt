...
class AnkiActivityHelpActionsDispatcher(
    ...
    override fun onSendReport() {
        if (AdaptionUtil.isUserATestClient) {
            ankiActivity.showSnackbar(ankiActivity.getString(CommonString.user_is_a_robot))
        } else {
            val wasReportSent = CrashReportService.sendReport(ankiActivity)
            if (!wasReportSent) {
                ankiActivity.showSnackbar(ankiActivity.getString(CommonString.help_dialog_exception_report_debounce))
                return
            }
            ankiActivity.showSnackbar(ankiActivity.getString(CommonString.help_dialog_exception_report_sent))
        }
    }
}
