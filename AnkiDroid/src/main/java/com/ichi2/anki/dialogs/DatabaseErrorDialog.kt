...
class DatabaseErrorDialog : AsyncDialogFragment() {
    ...
    override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
        ...
        return when (requireDialogType()) {
            DIALOG_LOAD_FAILED -> {
                ...
                // the activity
                alertDialog.show {
                    title(CommonString.open_collection_failed_title)
                    cancelable(false)
                    message(text = message)
                    setIcon(R.drawable.ic_warning)
                    positiveButton(CommonString.error_handling_options) {
                        showDatabaseErrorDialog(DIALOG_ERROR_HANDLING)
                    }
                    negativeButton(CommonString.close) {
            ...
            DIALOG_DB_ERROR -> {
                ...
                alertDialog
                    .create {
                        cancelable(false)
                        title(CommonString.answering_error_title)
                        message(text = message)
                        setIcon(R.drawable.ic_warning)
                        positiveButton(CommonString.error_handling_options) {
                            showDatabaseErrorDialog(DIALOG_ERROR_HANDLING)
                        }
                        negativeButton(CommonString.answering_error_report) {
                            requireDeckPicker().sendErrorReport()
                            requireActivity().dismissAllDialogFragments()
                        }
                        neutralButton(CommonString.close) {
                            closeCollectionAndFinish()
                        }
                    }.apply {
                        show()
            ...
            DIALOG_ERROR_HANDLING -> {
                ...
                if (!requireAnkiActivity().colIsOpenUnsafe()) {
                    // retry
                    options.add(res.getString(CommonString.backup_retry_opening))
                    values.add(ErrorHandlingEntries.RETRY)
                }
                // repair db with sqlite
                if (sqliteInstalled) {
                    options.add(res.getString(CommonString.backup_error_menu_repair))
                    values.add(ErrorHandlingEntries.REPAIR)
                }
                // // restore from backup
                options.add(res.getString(CommonString.backup_restore))
                values.add(ErrorHandlingEntries.RESTORE)
                // one-way sync from server
                if (isLoggedIn) {
                    options.add(res.getString(CommonString.backup_one_way_sync_from_server))
                ...
                if (shouldOfferResetToDefaultDirectory) {
                    options.add(res.getString(CommonString.backup_use_default_folder))
                    values.add(ErrorHandlingEntries.RESET_TO_DEFAULT_DIRECTORY)
                }

                // delete old collection and build new one
                options.add(res.getString(CommonString.backup_del_collection))
                ...
                alertDialog.show {
                    title(CommonString.error_handling_title)
                    setIcon(R.drawable.ic_warning)
            ...
            DIALOG_REPAIR_COLLECTION -> {
                ...
                alertDialog.show {
                    title(CommonString.dialog_positive_repair)
                    message(text = message)
                    setIcon(R.drawable.ic_warning)
                    positiveButton(CommonString.dialog_positive_repair) {
            ...
            DIALOG_RESTORE_BACKUP -> {
                ...
                if (backups.isEmpty()) {
                    alertDialog
                        .title(CommonString.backup_restore)
                        .title(text = message)
                    ...
                    alertDialog
                        .title(CommonString.backup_restore_select_title)
                        .positiveButton(CommonString.restore_backup_choose_another) {
                            ankiActivity?.let {
                        ...
                        .setSingleChoiceItems(dates.toTypedArray(), -1) { _, index: Int ->
                            if (backups[index].length() > 0) {
                                ...
                                AlertDialog.Builder(requireActivity()).show {
                                    ...
                                    message(CommonString.backup_invalid_file_error)
                                    positiveButton(CommonString.dialog_ok)
                                }
                            }
                        }
            ...
            DIALOG_NEW_COLLECTION -> {
                ...
                alertDialog.show {
                    title(CommonString.backup_new_collection)
                    message(text = message)
            ...
            DIALOG_ONE_WAY_SYNC_FROM_SERVER -> {
                ...
                alertDialog.show {
                    title(CommonString.backup_one_way_sync_from_server)
                    message(text = message)
                    positiveButton(CommonString.dialog_positive_overwrite) {
            ...
            INCOMPATIBLE_DB_VERSION -> {
                ...
                options.add(makeBold(res.getString(CommonString.backup_restore)))
                values.add(IncompatibleDbVersionEntries.RESTORE)
                if (isLoggedIn) {
                    options.add(makeBold(res.getString(CommonString.backup_one_way_sync_from_server)))
                    values.add(IncompatibleDbVersionEntries.ONE_WAY)
                }
                alertDialog.show {
                    cancelable(false)
                    title(CommonString.incompatible_database_version_title)
                    setIcon(R.drawable.ic_warning)
                    positiveButton(CommonString.close) {
                        closeCollectionAndFinish()
                    }
            ...
            DIALOG_STORAGE_UNAVAILABLE_AFTER_UNINSTALL -> {
                ...
                alertDialog.show {
                    title(CommonString.directory_inaccessible_after_uninstall)
    ...
    enum class UninstallListItem(
        ...
        RESTORE_FROM_ANKIWEB(
            CommonString.restore_data_from_ankiweb,
        ...
        INSTALL_NON_PLAY_APP_RECOMMENDED(
            CommonString.install_non_play_store_ankidroid_recommended,
        ...
        INSTALL_NON_PLAY_APP_NORMAL(
            CommonString.install_non_play_store_ankidroid,
            dismissesDialog = false,
            {
                it.openUrl(R.string.link_install_non_play_store_install)
            },
        ...
        RESTORE_FROM_BACKUP(
            CommonString.restore_data_from_backup,
            dismissesDialog = false,
            { activity ->
        ...
        GET_HELP(
            CommonString.help_title_get_help,
            dismissesDialog = false,
            {
        ...
        RECREATE_COLLECTION(
            CommonString.create_new_collection,
            dismissesDialog = false,
            {
                this.displayCreateNewCollectionDialog(it)
            },
        ...
        companion object {
            ...
            fun displayCreateNewCollectionDialog(context: AnkiActivity) {
                ...
                AlertDialog.Builder(context).show {
                    title(CommonString.backup_new_collection)
                    setIcon(R.drawable.ic_warning)
                    message(CommonString.new_unsafe_collection)
                    positiveButton(CommonString.dialog_positive_create) {
    ...
    private val message: String?
        get() =
            when (requireDialogType()) {
                ...
                DIALOG_REPAIR_COLLECTION -> res().getString(CommonString.repair_deck_dialog, BackupManager.BROKEN_COLLECTIONS_SUFFIX)
                DIALOG_RESTORE_BACKUP -> res().getString(CommonString.backup_restore_no_backups)
                DIALOG_NEW_COLLECTION -> res().getString(CommonString.backup_del_collection_question)
                ...
                INCOMPATIBLE_DB_VERSION -> {
                    ...
                    res().getString(
                        CommonString.incompatible_database_version_summary,
                        schemaVersion,
                        databaseVersion,
                    )
                }
                DIALOG_STORAGE_UNAVAILABLE_AFTER_UNINSTALL -> {
                    val directory =
                        context?.let { CollectionHelper.getCurrentAnkiDroidDirectory(it) }
                            ?: res().getString(CommonString.card_browser_unknown_deck_name)
                    res().getString(CommonString.directory_inaccessible_after_uninstall_summary, directory)
                }
    ...
    private val title: String
        get() =
            when (requireDialogType()) {
                DIALOG_LOAD_FAILED -> res().getString(CommonString.open_collection_failed_title)
                DIALOG_ERROR_HANDLING -> res().getString(CommonString.error_handling_title)
                DIALOG_REPAIR_COLLECTION -> res().getString(CommonString.dialog_positive_repair)
                DIALOG_RESTORE_BACKUP -> res().getString(CommonString.backup_restore)
                DIALOG_NEW_COLLECTION -> res().getString(CommonString.backup_new_collection)
                DIALOG_CONFIRM_DATABASE_CHECK -> TR.sentenceCase.checkDatabase
                DIALOG_CONFIRM_RESTORE_BACKUP -> res().getString(CommonString.restore_backup_title)
                DIALOG_ONE_WAY_SYNC_FROM_SERVER -> res().getString(CommonString.backup_one_way_sync_from_server)
                DIALOG_DB_LOCKED -> res().getString(CommonString.database_locked_title)
                INCOMPATIBLE_DB_VERSION -> res().getString(CommonString.incompatible_database_version_title)
                DIALOG_DB_ERROR -> res().getString(CommonString.answering_error_title)
                DIALOG_DISK_FULL -> res().getString(CommonString.storage_full_title)
                DIALOG_STORAGE_UNAVAILABLE_AFTER_UNINSTALL -> res().getString(CommonString.directory_inaccessible_after_uninstall)
            }

    override val notificationMessage: String? get() = message
    override val notificationTitle: String get() = res().getString(CommonString.answering_error_title)
