...
class DatabaseErrorDialog : AsyncDialogFragment() {
    ...
    override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
        ...
        return when (requireDialogType()) {
            DIALOG_LOAD_FAILED -> {
                ...
                // the activity
                alertDialog.show {
                    title(S.open_collection_failed_title)
                    cancelable(false)
                    message(text = message)
                    setIcon(R.drawable.ic_warning)
                    positiveButton(S.error_handling_options) {
            ...
            DIALOG_DB_ERROR -> {
                ...
                alertDialog
                    .create {
                        cancelable(false)
                        title(S.answering_error_title)
                        message(text = message)
                        setIcon(R.drawable.ic_warning)
                        positiveButton(S.error_handling_options) {
                            showDatabaseErrorDialog(DIALOG_ERROR_HANDLING)
                        }
                        negativeButton(S.answering_error_report) {
                            requireDeckPicker().sendErrorReport()
                            requireActivity().dismissAllDialogFragments()
                        }
                        neutralButton(S.close) {
                            closeCollectionAndFinish()
                        }
                    }.apply {
                        show()
            ...
            DIALOG_ERROR_HANDLING -> {
                ...
                if (!requireAnkiActivity().colIsOpenUnsafe()) {
                    // retry
                    options.add(res.getString(S.backup_retry_opening))
                    values.add(ErrorHandlingEntries.RETRY)
                }
                // repair db with sqlite
                if (sqliteInstalled) {
                    options.add(res.getString(S.backup_error_menu_repair))
                    values.add(ErrorHandlingEntries.REPAIR)
                }
                // // restore from backup
                options.add(res.getString(S.backup_restore))
                values.add(ErrorHandlingEntries.RESTORE)
                // one-way sync from server
                if (isLoggedIn) {
                    options.add(res.getString(S.backup_one_way_sync_from_server))
                ...
                if (shouldOfferResetToDefaultDirectory) {
                    options.add(res.getString(S.backup_use_default_folder))
                    values.add(ErrorHandlingEntries.RESET_TO_DEFAULT_DIRECTORY)
                }

                // delete old collection and build new one
                options.add(res.getString(S.backup_del_collection))
            ...
            DIALOG_REPAIR_COLLECTION -> {
                ...
                alertDialog.show {
                    title(S.dialog_positive_repair)
                    message(text = message)
                    setIcon(R.drawable.ic_warning)
                    positiveButton(S.dialog_positive_repair) {
            ...
            DIALOG_RESTORE_BACKUP -> {
                ...
                if (backups.isEmpty()) {
                    ...
                    alertDialog
                        .title(S.backup_restore_select_title)
                        .positiveButton(S.restore_backup_choose_another) {
                            ankiActivity?.let {
                        ...
                        .setSingleChoiceItems(dates.toTypedArray(), -1) { _, index: Int ->
                            if (backups[index].length() > 0) {
                                ...
                                AlertDialog.Builder(requireActivity()).show {
                                    title(S.vague_error)
                                    message(S.backup_invalid_file_error)
                                    positiveButton(S.dialog_ok)
                                }
                            }
                        }
            ...
            DIALOG_ONE_WAY_SYNC_FROM_SERVER -> {
                ...
                alertDialog.show {
                    title(S.backup_one_way_sync_from_server)
                    message(text = message)
                    positiveButton(S.dialog_positive_overwrite) {
            ...
            DIALOG_DB_LOCKED -> {
                ...
                alertDialog.show {
                    title(S.database_locked_title)
                    message(text = message)
                    positiveButton(S.close) {
                        closeCollectionAndFinish()
                    }
                    cancelable(false)
                }
            ...
            INCOMPATIBLE_DB_VERSION -> {
                ...
                options.add(makeBold(res.getString(S.backup_restore)))
                values.add(IncompatibleDbVersionEntries.RESTORE)
                if (isLoggedIn) {
                    options.add(makeBold(res.getString(S.backup_one_way_sync_from_server)))
                    values.add(IncompatibleDbVersionEntries.ONE_WAY)
                }
                alertDialog.show {
                    cancelable(false)
                    title(S.incompatible_database_version_title)
                    setIcon(R.drawable.ic_warning)
                    positiveButton(S.close) {
                        closeCollectionAndFinish()
                    }
            ...
            DIALOG_STORAGE_UNAVAILABLE_AFTER_UNINSTALL -> {
                ...
                alertDialog.show {
                    title(S.directory_inaccessible_after_uninstall)
    ...
    enum class UninstallListItem(
        ...
        INSTALL_NON_PLAY_APP_RECOMMENDED(
            S.install_non_play_store_ankidroid_recommended,
        ...
        INSTALL_NON_PLAY_APP_NORMAL(
            S.install_non_play_store_ankidroid,
            dismissesDialog = false,
            {
                it.openUrl(R.string.link_install_non_play_store_install)
            },
        ...
        companion object {
            ...
            fun displayCreateNewCollectionDialog(context: AnkiActivity) {
                ...
                AlertDialog.Builder(context).show {
                    title(S.backup_new_collection)
                    setIcon(R.drawable.ic_warning)
                    message(S.new_unsafe_collection)
                    positiveButton(S.dialog_positive_create) {
            ...
            when (requireDialogType()) {
                DIALOG_LOAD_FAILED ->
                    if (DatabaseCorruption.isDetected) {
                        // The sqlite database has been corrupted (DatabaseErrorHandler.onCorrupt() was called)
                        // Show a specific message appropriate for the situation
                        res().getString(S.corrupt_db_message, res().getString(R.string.repair_deck))
                    } else {
                        // Generic message shown when a libanki task failed
                        res().getString(S.access_collection_failed_message, res().getString(R.string.link_help))
                    }
                DIALOG_DB_ERROR -> res().getString(S.answering_error_message)
                DIALOG_DISK_FULL -> res().getString(S.storage_full_message)
                DIALOG_REPAIR_COLLECTION -> res().getString(S.repair_deck_dialog, BackupManager.BROKEN_COLLECTIONS_SUFFIX)
                DIALOG_RESTORE_BACKUP -> res().getString(S.backup_restore_no_backups)
                DIALOG_NEW_COLLECTION -> res().getString(S.backup_del_collection_question)
                DIALOG_CONFIRM_DATABASE_CHECK -> res().getString(S.check_db_warning)
                DIALOG_CONFIRM_RESTORE_BACKUP -> res().getString(S.restore_backup)
                DIALOG_ONE_WAY_SYNC_FROM_SERVER -> res().getString(S.backup_full_sync_from_server_question)
                DIALOG_DB_LOCKED -> res().getString(S.database_locked_summary)
                INCOMPATIBLE_DB_VERSION -> {
                    ...
                    res().getString(
                        S.incompatible_database_version_summary,
                        schemaVersion,
                        databaseVersion,
                    )
                }
                DIALOG_STORAGE_UNAVAILABLE_AFTER_UNINSTALL -> {
                    val directory =
                        context?.let { CollectionHelper.getCurrentAnkiDroidDirectory(it) }
                            ?: res().getString(S.card_browser_unknown_deck_name)
                    res().getString(S.directory_inaccessible_after_uninstall_summary, directory)
                }
    ...
    private val title: String
        get() =
            when (requireDialogType()) {
                DIALOG_LOAD_FAILED -> res().getString(S.open_collection_failed_title)
                DIALOG_ERROR_HANDLING -> res().getString(S.error_handling_title)
                DIALOG_REPAIR_COLLECTION -> res().getString(S.dialog_positive_repair)
                DIALOG_RESTORE_BACKUP -> res().getString(S.backup_restore)
                DIALOG_NEW_COLLECTION -> res().getString(S.backup_new_collection)
                DIALOG_CONFIRM_DATABASE_CHECK -> TR.sentenceCase.checkDatabase
                DIALOG_CONFIRM_RESTORE_BACKUP -> res().getString(S.restore_backup_title)
                DIALOG_ONE_WAY_SYNC_FROM_SERVER -> res().getString(S.backup_one_way_sync_from_server)
                DIALOG_DB_LOCKED -> res().getString(S.database_locked_title)
                INCOMPATIBLE_DB_VERSION -> res().getString(S.incompatible_database_version_title)
                DIALOG_DB_ERROR -> res().getString(S.answering_error_title)
                DIALOG_DISK_FULL -> res().getString(S.storage_full_title)
                DIALOG_STORAGE_UNAVAILABLE_AFTER_UNINSTALL -> res().getString(S.directory_inaccessible_after_uninstall)
            }

    override val notificationMessage: String? get() = message
    override val notificationTitle: String get() = res().getString(S.answering_error_title)
