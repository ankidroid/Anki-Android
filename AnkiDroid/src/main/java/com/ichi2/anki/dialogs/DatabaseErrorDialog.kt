...
class DatabaseErrorDialog : AsyncDialogFragment() {
    ...
    override fun onCreateDialog(savedInstanceState: Bundle?): MaterialDialog {
        ...
        val mType = requireArguments().getInt("dialogType")
        val res = resources
        val builder: MaterialDialog.Builder? = activity?.let { MaterialDialog.Builder(it) }
        ...
        return when (mType) {
            DIALOG_LOAD_FAILED -> {
                ...
                builder?.cancelable(false)
                    ...
                    ?.onPositive(
                        MaterialDialog.SingleButtonCallback { inner_dialog: MaterialDialog?, which: DialogAction? ->
                            (activity as DeckPicker?)
                                ?.showDatabaseErrorDialog(DIALOG_ERROR_HANDLING)
                        }
                    )
                    ?.onNegative(MaterialDialog.SingleButtonCallback { inner_dialog: MaterialDialog?, which: DialogAction? -> exit() })
                    ?.show()!!
            }
            DIALOG_DB_ERROR -> {
                ...
                val dialog: MaterialDialog = message?.let {
                    builder
                        ?.cancelable(false)
                        ?.content(it)
                        ?.iconAttr(R.attr.dialogErrorIcon)
                        ...
                        ?.onNeutral(MaterialDialog.SingleButtonCallback { inner_dialog: MaterialDialog?, which: DialogAction? -> exit() })
                        ?.show()
                }!!
                (activity as DeckPicker?)?.let {
                    dialog.getCustomView()?.findViewById<View>(R.id.md_buttonDefaultNegative)?.setEnabled(
                        it.hasErrorFiles()
                    )
                }
                dialog
            ...
            DIALOG_ERROR_HANDLING -> {
                ...
                if ((activity as AnkiActivity?)?.colIsOpen() == false) {
                    // retry
                ...
                builder?.iconAttr(R.attr.dialogErrorIcon)
                    ...
                    ?.items(*titles)
                    ?.itemsCallback(
                        MaterialDialog.ListCallback { materialDialog: MaterialDialog?, view: View?, which: Int, charSequence: CharSequence? ->
            ...
            DIALOG_REPAIR_COLLECTION -> {
                ...
                builder?.content(message.toString())
                    ?.iconAttr(R.attr.dialogErrorIcon)
            ...
            DIALOG_RESTORE_BACKUP -> {
                ...
                val path: String = CollectionHelper.getCollectionPath(activity)
                ...
                if (mBackups.size == 0) {
                    ...
                    var i = 0
                    while (i < mBackups.size) {
                        dates[i] = mBackups[i]!!.name.replace(
                    ...
                    builder?.title(res.getString(R.string.backup_restore_select_title))
                        ...
                        ?.itemsCallbackSingleChoice(
                            dates.size,
                            MaterialDialog.ListCallbackSingleChoice { materialDialog: MaterialDialog?, view: View?, which: Int, charSequence: CharSequence? ->
                ...
                val materialDialog: MaterialDialog = builder?.build()!!
                materialDialog.setOnKeyListener(
            ...
            DIALOG_NEW_COLLECTION -> {
                ...
                builder?.content(message.toString())
                    ...
                    ?.onPositive(
                        MaterialDialog.SingleButtonCallback { inner_dialog: MaterialDialog?, which: DialogAction? ->
                            val ch: CollectionHelper = CollectionHelper.getInstance()
                            val time: Time = ch.getTimeSafe(context)
                            ch.closeCollection(false, "DatabaseErrorDialog: Before Create New Collection")
                            val path1: String = CollectionHelper.getCollectionPath(activity)
            ...
            DIALOG_DB_LOCKED -> {
                ...
                builder?.content(message.toString())
                    ...
                    ?.cancelable(false)
                    ?.onPositive(MaterialDialog.SingleButtonCallback { inner_dialog: MaterialDialog?, which: DialogAction? -> exit() })
                    ?.show()!!
            }
            INCOMPATIBLE_DB_VERSION -> {
                val values: MutableList<Int> = ArrayList(2)
                val options = arrayOf<CharSequence>(UiUtil.makeBold(res.getString(R.string.backup_restore)), UiUtil.makeBold(res.getString(R.string.backup_full_sync_from_server)))
                values.add(0)
                values.add(1)
                builder
                    ?.cancelable(false)
                    ...
                    ?.itemsCallback(
                        MaterialDialog.ListCallback { dialog: MaterialDialog?, itemView: View?, position: Int, text: CharSequence? ->
    ...
    private val message: String?
        private get() = when (requireArguments().getInt("dialogType")) {
