...
class AnkiProgressDialogViewModel(
    ...
    val cancelableViaBackButton = MutableStateFlow(false)

    val cancelButtonText = MutableStateFlow<String?>(null)

    private var onCancelListener: (() -> Unit)? = null

    init {
        message.value = savedStateHandle.get<String>(KEY_MESSAGE) ?: ""
        cancelableViaBackButton.value = savedStateHandle.get<Boolean>(KEY_CANCELABLE) ?: false
        cancelButtonText.value = savedStateHandle.get<String>(KEY_CANCEL_BUTTON_TEXT)
        progress.value = savedStateHandle.get<Progress>(KEY_MESSAGE)
    }
    ...
    fun setup(
        ...
        this.cancelableViaBackButton.value = cancelableViaBackButton
