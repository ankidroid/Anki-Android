...
class SyncErrorDialog : AsyncDialogFragment() {
    interface SyncErrorDialogListener {
        ...
        fun sync()
        fun sync(conflict: Connection.ConflictResolution?)
        fun getCol(): Collection
        fun mediaCheck()
    ...
    override fun onCreateDialog(savedInstanceState: Bundle?): MaterialDialog {
        ...
        val builder: MaterialDialog.Builder = activity?.let {
        ...
        return when (requireArguments().getInt("dialogType")) {
            DIALOG_USER_NOT_LOGGED_IN_SYNC -> {
                ...
                builder.iconAttr(R.attr.dialogSyncErrorIcon)
                    ...
                    .onPositive(MaterialDialog.SingleButtonCallback { dialog: MaterialDialog?, which: DialogAction? -> (activity as SyncErrorDialogListener?)!!.loginToSyncServer() })
                    .show()
            }
            DIALOG_CONNECTION_ERROR -> {
                ...
                builder.iconAttr(R.attr.dialogSyncErrorIcon)
                    ...
                    .onNegative(MaterialDialog.SingleButtonCallback { dialog: MaterialDialog?, which: DialogAction? -> dismissAllDialogFragments() })
                    .show()
            }
            DIALOG_SYNC_CONFLICT_RESOLUTION -> {
                ...
                builder.iconAttr(R.attr.dialogSyncErrorIcon)
                    ...
                    .onNeutral(MaterialDialog.SingleButtonCallback { dialog: MaterialDialog?, which: DialogAction? -> dismissAllDialogFragments() })
                    .show()
            }
            ...
            DIALOG_SYNC_SANITY_ERROR_CONFIRM_KEEP_LOCAL -> {
                ...
                builder.positiveText(res().getString(R.string.dialog_positive_replace))
                    ...
                    .onPositive(
                        MaterialDialog.SingleButtonCallback { dialog: MaterialDialog?, which: DialogAction? ->
                            (activity as SyncErrorDialogListener?)!!.sync(Connection.ConflictResolution.FULL_UPLOAD)
                            dismissAllDialogFragments()
                        }
                    )
                    .show()
            ...
            DIALOG_SYNC_SANITY_ERROR_CONFIRM_KEEP_REMOTE -> {
                ...
                builder.positiveText(res().getString(R.string.dialog_positive_replace))
                    ...
                    .onPositive(
                        MaterialDialog.SingleButtonCallback { dialog: MaterialDialog?, which: DialogAction? ->
                            (activity as SyncErrorDialogListener?)!!.sync(Connection.ConflictResolution.FULL_DOWNLOAD)
                            dismissAllDialogFragments()
                        }
                    )
                    .show()
            ...
            DIALOG_SYNC_CORRUPT_COLLECTION -> {
                builder.positiveText(R.string.dialog_ok)
                    ...
                    .onNeutral(MaterialDialog.SingleButtonCallback { dialog: MaterialDialog?, which: DialogAction? -> (requireActivity() as AnkiActivity).openUrl(Uri.parse(getString(R.string.repair_deck))) })
                    .cancelable(false)
                    .show()
            }
            else -> {
