...
/**
 * An identifier for a row in the Card Browser, which may either be a [CardId] or a [NoteId]
 *
...
class BrowserRowCollection(
    var cardsOrNotes: CardsOrNotes,
    val cardOrNoteIdList: MutableList<CardOrNoteId>
) : MutableList<CardOrNoteId> by cardOrNoteIdList {

    fun replaceWith(cardsOrNotes: CardsOrNotes, newList: List<CardOrNoteId>) {
    ...
    suspend fun queryFirstCardIdPerNote(): List<CardId> = when (this.cardsOrNotes) {
        // TODO: This is slower than necessary
        CardsOrNotes.NOTES -> requireNoteIdList().map { nid ->
            CollectionManager.withCol {
