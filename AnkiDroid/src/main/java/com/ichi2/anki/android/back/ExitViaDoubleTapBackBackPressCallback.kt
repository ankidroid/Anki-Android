    ...
    object : OnBackPressedCallback(enabled = exitViaDoubleTapBack) {
        ...
        callback.strongListenerReference =
            OnSharedPreferenceChangeListener { prefs, key ->
                if (key == PREF_KEY) {
                    callback.isEnabled =
                        prefs.getBoolean(PREF_KEY, false).also {
...
private const val PREF_KEY = "exitViaDoubleTapBack"

/** Preference: Whether the user wants press back twice to return to the main screen"  */
private val exitViaDoubleTapBack
    get() = AnkiDroidApp.sharedPrefs().getBoolean(PREF_KEY, false)
