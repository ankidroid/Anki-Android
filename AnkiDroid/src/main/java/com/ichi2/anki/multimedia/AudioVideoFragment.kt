...
class AudioVideoFragment : MultimediaFragment(R.layout.fragment_audio_video) {
    ...
    override val title: String
        get() = selectedMediaOptions.toTitle(requireContext())
    ...
    // defaultArtwork(Unstable API) is required for audio files cover otherwise it black
     ...
     *
     * The intent's action is set to `Intent.ACTION_GET_CONTENT` to allow the user to pick an existing file.
     * Additionally, the category `Intent.CATEGORY_OPENABLE` is added to ensure only openable files are selectable (avoids issues with virtual files on Android 7+).
     *
     * Finally, the method retrieves the prompt string from resources using the provided `prompt` resource ID and launches the media chooser using the `pickMediaLauncher` with a chooser dialog displaying the retrieved prompt.
     *
    ...
    private fun handleMediaSelection(data: Intent) {
        ...
        viewModel.currentMultimediaUri = selectedMediaClip
     ...
     *
     * This method takes an array of strings, `mediaClipFullNameParts`, which is expected to contain
     * the filename and extension of the media clip.
     * It attempts to create a cached file using `createCachedFile` with the following arguments:
     *   * Filename: Concatenation of the first element (filename) and second element (extension) from `mediaClipFullNameParts` separated by a dot (.).
     *   * Directory: The `ankiCacheDirectory` constant.
     *
     ...
     *
     * This method takes a `Uri` representing the selected media clip and a `File` object representing the
     * created temporary file as input. In case of any exceptions during the copy process,it sends an
     * exception report, displays an error message, and does not modify the view model or UI.
     *
     * @param selectedMediaClip The Uri representing the selected media clip.
    ...
    private fun copyMediaFileToTemp(selectedMediaClip: Uri, clipCopy: File) {
        try {
            requireContext().contentResolver.openInputStream(selectedMediaClip).use { inputStream ->
                ...
                viewModel.currentMultimediaPath = clipCopy.path
    ...
    /**
     * Function for the [MediaOption] enum that returns the appropriate title string
     * based on the current media option.
     *
     * @receiver MediaOption The media option for which the title string is to be returned.
     ...
     */
    fun MediaOption.toTitle(context: Context): String {
        return when (this) {
