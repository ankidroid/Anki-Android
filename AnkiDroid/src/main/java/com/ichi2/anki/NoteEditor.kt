...
class NoteEditor : AnkiActivity(), DeckSelectionListener, SubtitleListener, TagsDialogListener {
        ...
        if (addNote) {
            ...
            val noOfAddedCards = withProgress(resources.getString(R.string.saving_facts)) {
                CollectionManager.withCol {
                    models.current(col)!!.put("tags", tags)
                    models.setChanged()
    ...
    private fun changeNoteType(oldModel: Model?, newModel: Model?) {
        ...
        col.models.change(col, oldModel!!, noteId, newModel!!, mModelChangeFieldMap!!, mModelChangeCardMap!!)
    ...
    private val currentlySelectedModel: Model?
        get() = col.models.get(col, mAllModelIds!![mNoteTypeSpinner!!.selectedItemPosition])
    ...
    private inner class SetNoteTypeListener : OnItemSelectedListener {
        override fun onItemSelected(parent: AdapterView<*>?, view: View?, pos: Int, id: Long) {
            ...
            val oldModelId = col.models.current(col)!!.getLong("id")
            val newId = mAllModelIds!![pos]
            Timber.i("Changing note type to '%d", newId)
            if (oldModelId != newId) {
                val model = col.models.get(col, newId)
                if (model == null) {
    ...
    private inner class EditNoteTypeListener : OnItemSelectedListener {
        override fun onItemSelected(parent: AdapterView<*>?, view: View?, pos: Int, id: Long) {
            ...
            // Get new model
            val newModel = col.models.get(col, mAllModelIds!![pos])
            if (newModel == null) {
