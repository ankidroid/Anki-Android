...
open class MouseContextMenuHandler(
    ...
    fun showContextMenu(
        ...
        val anchorView =
            View(context).apply {
                // Position the anchor at the mouse coordinates with slight offset
                this.x = x + 10f
                this.y = y + 10f
                this.layoutParams = ViewGroup.LayoutParams(1, 1)
            }

        // Add the anchor to the parent view temporarily
        ...
        // Populate menu using the provided content provider
        menuContentProvider.populateMenu(popupMenu.menu)

        // Allow content provider to prepare/update menu based on current state
        menuContentProvider.onPrepareMenu(popupMenu.menu)

        // Set click listener for menu items using provider's logic
