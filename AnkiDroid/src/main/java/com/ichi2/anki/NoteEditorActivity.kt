    ...
    fun loadNoteEditorPreviewer(forceReplace: Boolean) {
        ...
        lifecycleScope.launch {
            try {
                ...
                val shouldReplace = forceReplace
                ...
                if (shouldReplace) {
                ...
                } else if (existingPreviewer is TemplatePreviewerFragment) {
                    ...
                    if (noteEditorFragment.editorNote!!.notetype.isCloze) {
                        val currentOrd = existingPreviewer.getSafeClozeOrd()
                        val previewerFragment = createPreviewerFragment(fields, tags, currentOrd)
                        supportFragmentManager.commit {
                            replace(R.id.previewer_frame, previewerFragment)
                            runOnCommit {
                                configurePreviewerTabs(previewerFragment)
