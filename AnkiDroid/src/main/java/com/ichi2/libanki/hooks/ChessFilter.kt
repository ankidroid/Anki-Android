...
object ChessFilter {
    ...
    private val fFenOrientationPattern = Pattern.compile("orientation *= *\"?(black|white)\"?")
    private const val fRenderFen = "(function (fentxt, showBlack) {" +
    ...
    fun fenToChessboard(text: String, context: Context?): String {
        ...
        while (mf.find()) {
            if (mf.group(1) != null) {
                ...
                if (mo.find() && mo.group(1) != null && "black".equals(mo.group(1), ignoreCase = true)) {
                    showBlack = true
                }
            }
            try {
