...
open class Collection @VisibleForTesting constructor(
    ...
    init {
        ...
        load()
        if (crt == 0L) {
            crt = UIUtils.getDayStart(clock) / 1000
        }
        mStartReps = 0
        mStartTime = 0
        _loadScheduler()
    ...
    fun flush(mod: Long = 0) {
        ...
        this.mod = if (mod == 0L) clock.intTimeMS() else mod
...
//        _markOp(name);
    ...
    fun <T> genCards(
        ...
        var ts = clock.maxID(db)
        val now = clock.intTime()
        val rem =
    ...
    fun timeboxReached(): Pair<Int, Int>? {
        ...
        val elapsed = clock.intTime() - mStartTime
    ...
    fun log(vararg argsParam: Any?) {
        ...
        val s = String.format(
            "[%s] %s:%s(): %s", clock.intTime(), trace.fileName, trace.methodName,
            TextUtils.join(",  ", args)
        )
        writeLog(s)
    }
    ...
    fun crtCalendar(): Calendar {
        return Clock.calendar((crt * 1000).toLong())
    }

    fun crtGregorianCalendar(): GregorianCalendar {
        return Clock.gregorianCalendar((crt * 1000).toLong())
    }
