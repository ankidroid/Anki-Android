...
object Utils {
     ...
     */
    @KotlinCleanup("Replace with Kotlin functions")
    ...
    fun remainingTime(context: Context, time_s: Long): String {
        ...
        return if (time_s < TIME_HOUR_LONG) {
            ...
            remaining =
                Math.round(remainingSeconds.toFloat() / TIME_MINUTE)
                    .toInt()
            ...
            remaining =
                Math.round(remainingSeconds / TIME_HOUR).toInt()
            res.getQuantityString(
    ...
    fun timeSpan(context: Context, time_s: Long): String {
        ...
        return if (Math.abs(time_s) < TIME_MINUTE) {
            timeX = time_s.toInt()
            res.getQuantityString(R.plurals.time_span_seconds, timeX, timeX)
        } else if (Math.abs(time_s) < TIME_HOUR) {
            timeX = Math.round(time_s / TIME_MINUTE).toInt()
            res.getQuantityString(R.plurals.time_span_minutes, timeX, timeX)
        } else if (Math.abs(time_s) < TIME_DAY) {
            timeX = Math.round(time_s / TIME_HOUR).toInt()
            res.getQuantityString(R.plurals.time_span_hours, timeX, timeX)
        } else if (Math.abs(time_s) < TIME_MONTH) {
            timeX = Math.round(time_s / TIME_DAY).toInt()
            res.getQuantityString(R.plurals.time_span_days, timeX, timeX)
        } else if (Math.abs(time_s) < TIME_YEAR) {
            timeX = Math.round(time_s / TIME_MONTH).toInt()
            res.getQuantityString(R.plurals.time_span_months, timeX, timeX)
        } else {
            timeX = Math.round(time_s / TIME_YEAR).toInt()
            res.getQuantityString(R.plurals.time_span_years, timeX, timeX)
        }
    }
    ...
    * HTML
     ...
     */
    @Suppress("NAME_SHADOWING")
    fun stripHTMLScriptAndStyleTags(s: CharSequence): String {
        var s = s
     ...
     */
    @JvmOverloads
    fun stripSoundMedia(s: CharSequence, replacement: String = " $1 "): String {
     ...
     * This should only affect substrings of the form  and not tags.
    ...
    private fun base62(num: Int, extra: String): String {
        var num = num
    ...
    fun splitFields(fields: String): Array<String> {
        ...
        return fields.split(FIELD_SEPARATOR).dropLastWhile { it.isEmpty() }.toTypedArray()
    }
    /*
     * Checksums
     * ***********************************************************************************************
     ...
     */
    @KotlinCleanup("Redundant initializer for `numRead`")
    ...
    fun unzipFiles(
        ...
        zipEntryToFilenameMap: Map<String?, String?>?
    ) {
        var zipEntryToFilenameMap = zipEntryToFilenameMap
    ...
    @KotlinCleanup("isEmpty() -> isNotEmpty()")
    ...
    @KotlinCleanup("Replace with Kotlin function")
    ...
    */
    ...
    fun nonEmptyFields(fields: Map<String, String>): Set<String> {
        val nonemptyFields: MutableSet<String> = HashSetInit(fields.size)
        for (kv in fields.entries) {
            var value = kv.value
            value = stripHTMLMedia(value).trim { it <= ' ' }
            if (!TextUtils.isEmpty(value)) {
                nonemptyFields.add(kv.key)
            }
        }
