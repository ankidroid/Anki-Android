/*
 ...
 Ported from:
 https://github.com/python/cpython/blob/c88239f864a27f673c0f0a9e62d2488563f9d081/Modules/_csv.c
 */
...
//        }
    ...
    // Copied from C code
    private fun parse_process_char(_c: Char): Int {
        var c = _c
        val dialect = reader.dialect
        when (state) {
            State.START_RECORD -> {
                /* start of record */if (c == '\u0000') { /* empty line - return [] */
                } else if (c == '\n' || c == '\r') {
                    state = State.EAT_CRNL
                }
                /* normal character - handle as START_FIELD */state = State.START_FIELD
                /* expecting field */if (c == '\n' || c == '\r' || c == '\u0000') {
                    /* save empty field - return [fields] */
                    if (parse_save_field() < 0) return -1
                    state = if (c == '\u0000') State.START_RECORD else State.EAT_CRNL
                } else if (c == dialect.mQuotechar &&
                    dialect.mQuoting != Quoting.QUOTE_NONE
                ) {
                    /* start quoted field */
                ...
                } else if (c == ' ' && dialect.mSkipInitialSpace) { /*empty line*/
                }; else if (c == dialect.mDelimiter) {
                    /* save empty field */
                    if (parse_save_field() < 0) return -1
                } else {
                    /* begin new unquoted field */
                    if (dialect.mQuoting == Quoting.QUOTE_NONNUMERIC) numeric_field = 1
                    if (parse_add_char(c) < 0) return -1
                    state = State.IN_FIELD
                }
            }
            State.START_FIELD -> if (c == '\n' || c == '\r' || c == '\u0000') {
                if (parse_save_field() < 0) return -1
                state = if (c == '\u0000') State.START_RECORD else State.EAT_CRNL
            } else if (c == dialect.mQuotechar &&
                dialect.mQuoting != Quoting.QUOTE_NONE
            ...
            State.IN_QUOTED_FIELD -> /* in quoted field */if (c == '\u0000') { /*empty line*/
            }; else if (c == dialect.mEscapechar) {
            ...
            State.QUOTE_IN_QUOTED_FIELD -> /* doublequote - seen a quote in a quoted field */if (dialect.mQuoting != Quoting.QUOTE_NONE &&
            ...
            State.EAT_CRNL -> if (c == '\n' || c == '\r') { /*empty line*/
            }; else if (c == '\u0000') state = State.START_RECORD else {
