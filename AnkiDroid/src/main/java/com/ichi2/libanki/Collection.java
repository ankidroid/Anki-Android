...
//        _markOp(name);
    ...
    public void remCards(LongCollection ids, boolean notes) {
        ...
        LongArrayList nids = mDb.queryLongList("SELECT nid FROM cards WHERE id IN " + sids);
        // remove cards
    ...
    public LongArrayList findCards(String search) {
        return new Finder(this).findCards(search, null);
    }


    /** Return a list of card ids */
    public LongArrayList findCards(String search, String order) {
        return new Finder(this).findCards(search, order);
    }

    public LongArrayList findCards(String search, boolean order) {
        return findCards(search, order, null);
    }

    public LongArrayList findCards(String search, boolean order, CollectionTask.PartialSearch task) {
    ...
    private ArrayList<String> fixExcessiveReviewDueDates(Runnable notifyProgress) {
        ...
        LongArrayList ids = mDb.queryLongList("SELECT id FROM cards WHERE queue = " + Consts.QUEUE_TYPE_REV + " AND due > 100000");
        notifyProgress.run();
        if (ids.size() > 0) {
    ...
    private ArrayList<String> removeDynamicPropertyFromNonDynamicDecks(Runnable notifyProgress) {
        ...
        LongArrayList ids = mDb.queryLongList(
    ...
    /** Not in libAnki */
    @CheckResult
    public LongArrayList filterToValidCards(long[] cards) {
