...
@KotlinCleanup("Convert JvmFields to Kotlin Properties, remove redundant getters and setters")
class PlotSheet : Drawable {
    @JvmField
    @KotlinCleanup("Specify non null or nullable explicitly to avoid unchecked nullability issues")
    ...
    @JvmField
    @KotlinCleanup("Specify non null or nullable explicitly to avoid unchecked nullability issues")
    protected var textColor: ColorWrap = ColorWrap.black
    ...
    // if class should be made threadable for mulitplot mode, then
    ...
    /**
     * the plotting screens, screen 0 is the only one in single mode
     */
     ...
     */
    constructor(xStart: Double, xEnd: Double, yStart: Double, yEnd: Double, drawables: Vector<Drawable?>?) {
    ...
    fun xToGraphic(x: Double, field: RectangleWrap): Float {
        val xQuotient = (field.width - leftFrameThickness - rightFrameThickness) /
            @KotlinCleanup("Kotlin abs instead of Math.abs")
            Math.abs(
    ...
    override fun paint(g: GraphicsWrap) {
        ...
        if (screenParts[0].drawables != null && screenParts[0].drawables.size != 0) {
        ...
        if (leftFrameThickness > 0 || rightFrameThickness > 0 || upperFrameThickness > 0 || bottomFrameThickness > 0) {
            ...
            val keyList: MutableList<String?> = Vector(mLegendMap.keys)
            if (isBackwards) {
    ...
    private fun drawBorder(g: GraphicsWrap, field: RectangleWrap) {
        ...
        g.fillRect(
            leftFrameThickness - borderThickness + 1, upperFrameThickness - borderThickness + 1,
            field.width - leftFrameThickness - rightFrameThickness + 2 * borderThickness - 2, borderThickness.toFloat()
        )

        // lower border
        g.fillRect(
            leftFrameThickness - borderThickness + 1, field.height - bottomFrameThickness,
            field.width - leftFrameThickness - rightFrameThickness + 2 * borderThickness - 2, borderThickness.toFloat()
        )
        ...
        g.fillRect(
            leftFrameThickness - borderThickness + 1, upperFrameThickness - borderThickness + 1,
            borderThickness.toFloat(), field.height - upperFrameThickness - bottomFrameThickness + 2 * borderThickness - 2
        )

        // right border
        g.fillRect(
            field.width - rightFrameThickness, upperFrameThickness - borderThickness + 1,
            borderThickness.toFloat(), field.height - upperFrameThickness - bottomFrameThickness + 2 * borderThickness - 2
        )
    }
     ...
     *
     * @return double array in the length of two with the first element being the left and the second element being the right border
     */
    ...
    val frameThickness: FloatArray
        get() = floatArrayOf(leftFrameThickness, rightFrameThickness, upperFrameThickness, bottomFrameThickness)

    /**
     * set the size of the outer frame in pixel
     */
    fun setFrameThickness(leftFrameThickness: Float, rightFrameThickness: Float, upperFrameThickness: Float, bottomFrameThickness: Float) {
        if (leftFrameThickness < 0 || rightFrameThickness < 0 || upperFrameThickness < 0 || bottomFrameThickness < 0) {
    ...
    fun setTextColor(textColor: ColorWrap) {
